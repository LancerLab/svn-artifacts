"""M1 mutants over the reshape kernel (benchmark2/triton/kernels/reshape.py).
Interface: python3 reshape.py --family N  (see mutants/README.md)
"""
import argparse
import sys
...
@triton.jit
def copy_carrier(x_ptr, y_ptr, n, BLOCK: tl.constexpr):
...
@triton.jit
def copy_alias_rev(x_ptr, n, BLOCK: tl.constexpr):
    # M1.11: in-place read-after-write aliasing overlap (reversal).
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    tl.store(x_ptr + (n - 1 - offs),
             tl.load(x_ptr + offs, mask=mask), mask=mask)


@triton.jit
def copy_alias_shift(x_ptr, n, BLOCK: tl.constexpr):
    # M1.11 second realisation: forward-shift overlap.
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < n
    dst = offs + 1
    tl.store(x_ptr + dst, tl.load(x_ptr + offs, mask=mask), mask=dst < n)


FAMILIES = {19: copy_carrier}
...
def main():
    ...
    if args.family == 19:
        ...
        run = "ok"
        try:
            copy_carrier[(triton.cdiv(n, 1024),)](x, x, n, BLOCK=1024)
            sync()
    ...
    if args.family in (11, 111):
        ...
        n = FULL_RAGGED["reshape"][0]
        x0 = randn(n)
        x = GpuBuf.from_numpy(x0.copy())
        kern = copy_alias_rev if args.family == 11 else copy_alias_shift
        run = "ok"
        try:
        ...
        if got is None:
            ...
            out = "same" if np.array_equal(got, x0) else "diff"
