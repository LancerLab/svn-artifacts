module @f_4_dynamic_Nx256x56x56_256_256_Nx256x56x56 attributes {hal.device.targets = [#hal.device.target<"cuda", {executable_targets = [#hal.executable.target<"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}>], legacy_sync}>]} {
  hal.executable private @f_4_dynamic_Nx256x56x56_256_256_Nx256x56x56_dispatch_0 {
    hal.executable.variant public @cuda_nvptx_fb, target = <"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}> {
      hal.executable.export public @f_4_dynamic_Nx256x56x56_256_256_Nx256x56x56_dispatch_0_generic_Dx256x56x56_f32 ordinal(0) layout(#hal.pipeline.layout<push_constants = 1, sets = [<0, bindings = [<0, storage_buffer, ReadOnly>, <1, storage_buffer, ReadOnly>, <2, storage_buffer, ReadOnly>, <3, storage_buffer, ReadOnly>, <4, storage_buffer>]>]>) {
      ^bb0(%arg0: !hal.device, %arg1: index):
        %x, %y, %z = flow.dispatch.workgroup_count_from_slice %arg1
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @f_4_dynamic_Nx256x56x56_256_256_Nx256x56x56_dispatch_0_generic_Dx256x56x56_f32() {
          ...
          %7 = hal.interface.binding.subspan set(0) binding(4) type(storage_buffer) alignment(64) offset(%c0) : !flow.dispatch.tensor<writeonly:tensor<?x256x56x56xf32>>{%5}
          %8 = flow.dispatch.tensor.load %6, offsets = [0, 0, 0, 0], sizes = [%5, 256, 56, 56], strides = [1, 1, 1, 1] : !flow.dispatch.tensor<readonly:tensor<?x256x56x56xf32>>{%5} -> tensor<?x256x56x56xf32>
          ...
          %12 = tensor.empty(%5) : tensor<?x256x56x56xf32>
          %13 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1)>, affine_map<(d0, d1, d2, d3) -> (d1)>, affine_map<(d0, d1, d2, d3) -> (d1)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%8, %9, %10, %11 : tensor<?x256x56x56xf32>, tensor<256xf32>, tensor<256xf32>, tensor<256xf32>) outs(%12 : tensor<?x256x56x56xf32>) {
          ...
          flow.dispatch.tensor.store %13, %7, offsets = [0, 0, 0, 0], sizes = [%5, 256, 56, 56], strides = [1, 1, 1, 1] : tensor<?x256x56x56xf32> -> !flow.dispatch.tensor<writeonly:tensor<?x256x56x56xf32>>{%5}
          return
        }
      }
    }
  ...
  func.func @f_4_dynamic_Nx256x56x56_256_256_Nx256x56x56(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    ...
    %7 = stream.cmd.execute await(%result_timepoint) => with(%2 as %arg3: !stream.resource<external>{%1}, %3 as %arg4: !stream.resource<external>{%c1024}, %4 as %arg5: !stream.resource<external>{%c1024}, %5 as %arg6: !stream.resource<external>{%1}, %result as %arg7: !stream.resource<transient>{%c1024}) {
      ...
      stream.cmd.dispatch @f_4_dynamic_Nx256x56x56_256_256_Nx256x56x56_dispatch_0::@cuda_nvptx_fb::@f_4_dynamic_Nx256x56x56_256_256_Nx256x56x56_dispatch_0_generic_Dx256x56x56_f32[%0](%6 : i32) {
    ...
    %10 = stream.tensor.export %9 : tensor<?x256x56x56xf32>{%0} in !stream.resource<external>{%1} -> !hal.buffer_view
