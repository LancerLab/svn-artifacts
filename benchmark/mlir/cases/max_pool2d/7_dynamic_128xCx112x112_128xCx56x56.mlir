module {
  func.func @f_7_dynamic_128xCx112x112_128xCx56x56(%input: tensor<128x?x112x112xf32>) -> tensor<128x?x56x56xf32> {
    ...
    %po_sz2 = arith.constant 56 : index
    %po_sz3 = arith.constant 56 : index
    %t_res_pn = scf.for %pn = %c0 to %input_d0 step %c1 iter_args(%t_mp_pn = %out) -> (tensor<128x?x56x56xf32>) {
    %t_res_pc = scf.for %pc = %c0 to %input_d1 step %c1 iter_args(%t_mp_pc = %t_mp_pn) -> (tensor<128x?x56x56xf32>) {
    %t_res_poh = scf.for %poh = %c0 to %po_sz2 step %c1 iter_args(%t_mp_poh = %t_mp_pc) -> (tensor<128x?x56x56xf32>) {
    %t_res_pow = scf.for %pow = %c0 to %po_sz3 step %c1 iter_args(%t_mp_pow = %t_mp_poh) -> (tensor<128x?x56x56xf32>) {
    ...
    %pv    = tensor.extract %input[%pn, %pc, %ih, %iw] : tensor<128x?x112x112xf32>
    ...
    %t_new = tensor.insert %pool into %t_mp_pow[%pn, %pc, %poh, %pow] : tensor<128x?x56x56xf32>
    scf.yield %t_new : tensor<128x?x56x56xf32>
    }
    scf.yield %t_res_pow : tensor<128x?x56x56xf32>
    }
    scf.yield %t_res_poh : tensor<128x?x56x56xf32>
    }
    scf.yield %t_res_pc : tensor<128x?x56x56xf32>
    }
    return %t_res_pn : tensor<128x?x56x56xf32>
  }
}
