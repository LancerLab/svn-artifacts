module {
  func.func @f_13_dynamic_32x512xV_32x512xV(%input: tensor<32x512x?xf32>) -> tensor<32x512x?xf32> {
    ...
    %max_t = tensor.empty() : tensor<32x512xf32>
    %neg_inf = arith.constant -3.4028234663852886e+38 : f32
    %max_init = linalg.fill ins(%neg_inf : f32) outs(%max_t : tensor<32x512xf32>) -> tensor<32x512xf32>
    %max_result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>],
      iterator_types = ["parallel", "parallel", "reduction"]
    } ins(%input : tensor<32x512x?xf32>) outs(%max_init : tensor<32x512xf32>) {
    ^bb0(%in: f32, %cur: f32):
    ...
    %sum_result = linalg.generic {
      ...
      iterator_types = ["parallel", "parallel", "reduction"]
    } ins(%input, %max_result : tensor<32x512x?xf32>, tensor<32x512xf32>) outs(%sum_init : tensor<32x512xf32>) {
    ...
    } ins(%input, %max_result, %sum_result : tensor<32x512x?xf32>, tensor<32x512xf32>, tensor<32x512xf32>) outs(%out : tensor<32x512x?xf32>) {
