module {
  func.func @f_13_efficientnet_64x1280x7x7_64x1280(%input: tensor<64x1280x7x7xf32>) -> tensor<64x1280xf32> {
    ...
    } ins(%input : tensor<64x1280x7x7xf32>) outs(%sum_init : tensor<64x1280xf32>) {
    ^bb0(%in: f32, %acc: f32):
      %ns = arith.addf %acc, %in : f32
      linalg.yield %ns : f32
    } -> tensor<64x1280xf32>
    %scale = arith.constant 0.02040816326530612 : f32
    %div_t = tensor.empty() : tensor<64x1280xf32>
    %div_init = linalg.fill ins(%zero_f : f32) outs(%div_t : tensor<64x1280xf32>) -> tensor<64x1280xf32>
    %mean_result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>],
      iterator_types = ["parallel", "parallel"]
    } ins(%sum_result : tensor<64x1280xf32>) outs(%div_init : tensor<64x1280xf32>) {
    ^bb0(%sv: f32, %init: f32):
      %res = arith.mulf %sv, %scale : f32
      linalg.yield %res : f32
    } -> tensor<64x1280xf32>
    return %mean_result : tensor<64x1280xf32>
  }
}
