module {
  func.func @f_6_dynamic_Nx1280xMxK_Nx1280x1x1_Nx1280xMxK(%lhs: tensor<?x1280x?x?xf32>, %rhs: tensor<?x1280x1x1xf32>) -> tensor<?x1280x?x?xf32> {
    ...
    %t_res_ei0 = scf.for %ei0 = %c0 to %lhs_d0 step %c1 iter_args(%t_ea_ei0 = %out) -> (tensor<?x1280x?x?xf32>) {
    %t_res_ei1 = scf.for %ei1 = %c0 to %lhs_d1 step %c1 iter_args(%t_ea_ei1 = %t_ea_ei0) -> (tensor<?x1280x?x?xf32>) {
    %t_res_ei2 = scf.for %ei2 = %c0 to %lhs_d2 step %c1 iter_args(%t_ea_ei2 = %t_ea_ei1) -> (tensor<?x1280x?x?xf32>) {
    %t_res_ei3 = scf.for %ei3 = %c0 to %lhs_d3 step %c1 iter_args(%t_ea_ei3 = %t_ea_ei2) -> (tensor<?x1280x?x?xf32>) {
    %va = tensor.extract %lhs[%ei0, %ei1, %ei2, %ei3] : tensor<?x1280x?x?xf32>
    %vb = tensor.extract %rhs[%ei0, %ei1, %ei2, %ei3] : tensor<?x1280x1x1xf32>
    %rs = arith.addf %va, %vb : f32
    %t_ins = tensor.insert %rs into %t_ea_ei3[%ei0, %ei1, %ei2, %ei3] : tensor<?x1280x?x?xf32>
    scf.yield %t_ins : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_ei3 : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_ei2 : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_ei1 : tensor<?x1280x?x?xf32>
    }
    return %t_res_ei0 : tensor<?x1280x?x?xf32>
  }
}
