...
module {
  func.func @main() {
    ...
    %c1 = arith.constant 96 : index
    %c2 = arith.constant 112 : index
    %c3 = arith.constant 112 : index
    %cst_neg = arith.constant -2.5 : f32
    %inp = memref.alloc(%c0, %c1, %c2, %c3) : memref<?x?x?x?xf32>
    %out = memref.alloc(%c0, %c1, %c2, %c3) : memref<?x?x?x?xf32>
    linalg.fill ins(%cst_neg : f32) outs(%inp : memref<?x?x?x?xf32>)
    linalg.generic {
      indexing_maps = [#map, #map],
      iterator_types = ["parallel", "parallel", "parallel", "parallel"]
    } ins(%inp : memref<?x?x?x?xf32>) outs(%out : memref<?x?x?x?xf32>) {
    ...
    memref.dealloc %inp : memref<?x?x?x?xf32>
