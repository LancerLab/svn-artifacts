module @f_10_dynamic_16x512xHxW_16x512xHxW_16x512xHxW {
  flow.executable private @f_10_dynamic_16x512xHxW_16x512xHxW_16x512xHxW_dispatch_0 {
    flow.executable.export public @f_10_dynamic_16x512xHxW_16x512xHxW_16x512xHxW_dispatch_0_generic_16x512xDxD_f32 workgroups(%arg0: index, %arg1: index, %arg2: index, %arg3: index) -> (index, index, index) {
    ...
    builtin.module {
      func.func @f_10_dynamic_16x512xHxW_16x512xHxW_16x512xHxW_dispatch_0_generic_16x512xDxD_f32(%arg0: !flow.dispatch.tensor<readonly:tensor<16x512x?x?xf32>>, %arg1: !flow.dispatch.tensor<readonly:tensor<16x512x?x?xf32>>, %arg2: index, %arg3: index, %arg4: index, %arg5: index, %arg6: !flow.dispatch.tensor<writeonly:tensor<16x512x?x?xf32>>) {
        ...
        %6 = flow.dispatch.tie_shape %arg6 : !flow.dispatch.tensor<writeonly:tensor<16x512x?x?xf32>>{%2, %3}
        ...
        %9 = tensor.empty(%2, %3) : tensor<16x512x?x?xf32>
        %10 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%7, %8 : tensor<16x512x?x?xf32>, tensor<16x512x?x?xf32>) outs(%9 : tensor<16x512x?x?xf32>) {
        ^bb0(%in: f32, %in_0: f32, %out: f32):
          %11 = arith.addf %in, %in_0 : f32
          linalg.yield %11 : f32
        } -> tensor<16x512x?x?xf32>
        flow.dispatch.tensor.store %10, %6, offsets = [0, 0, 0, 0], sizes = [16, 512, %2, %3], strides = [1, 1, 1, 1] : tensor<16x512x?x?xf32> -> !flow.dispatch.tensor<writeonly:tensor<16x512x?x?xf32>>{%2, %3}
        return
      }
    }
  }
  func.func @f_10_dynamic_16x512xHxW_16x512xHxW_16x512xHxW(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    ...
    %6 = flow.dispatch @f_10_dynamic_16x512xHxW_16x512xHxW_16x512xHxW_dispatch_0::@f_10_dynamic_16x512xHxW_16x512xHxW_16x512xHxW_dispatch_0_generic_16x512xDxD_f32[%3, %4, %0, %1](%2, %5, %3, %4, %0, %1) : (tensor<16x512x?x?xf32>{%0, %1}, tensor<16x512x?x?xf32>{%3, %4}, index, index, index, index) -> tensor<16x512x?x?xf32>{%0, %1}
    %7 = hal.tensor.export %6 "output 0" : tensor<16x512x?x?xf32>{%0, %1} -> !hal.buffer_view
