module @f_21_vit_32x197x3072_3072_3072_32x197x3072 {
  func.func @f_21_vit_32x197x3072_3072_3072_32x197x3072(%input: memref<32x197x3072xf32>, %gamma: memref<3072xf32>, %beta: memref<3072xf32>) -> memref<32x197x3072xf32> {
    ...
    scf.for %bn0 = %c0 to %input_d0 step %c1 {
      scf.for %bn1 = %c0 to %input_d1 step %c1 {
        scf.for %bn2 = %c0 to %input_d2 step %c1 {
          %xv  = memref.load %input[%bn0, %bn1, %bn2] : memref<32x197x3072xf32>
          %gv  = memref.load %gamma[%bn2] : memref<3072xf32>
          %bv  = memref.load %beta[%bn2]  : memref<3072xf32>
          %scl = arith.mulf %xv, %gv : f32
          %res = arith.addf %scl, %bv : f32
          memref.store %res, %out[%bn0, %bn1, %bn2] : memref<32x197x3072xf32>
        }
      }
    }
