module @f_20_vit_32x3x224x224_32x3x112x112 {
  func.func @f_20_vit_32x3x224x224_32x3x112x112(%input: memref<32x3x224x224xf32>) -> memref<32x3x112x112xf32> {
    ...
    %out = memref.alloc() : memref<32x3x112x112xf32>
          ...
          scf.for %ow = %c0 to %out_d3 step %c1 {
            ...
            %max_v = scf.for %kh = %c0 to %kH_sz step %c1 iter_args(%mx_h = %neg_inf) -> (f32) {
              %max_v2 = scf.for %kw = %c0 to %kW_sz step %c1 iter_args(%mx_w = %mx_h) -> (f32) {
                ...
                %pv   = memref.load %input[%n, %c, %ih, %iw] : memref<32x3x224x224xf32>
            ...
            memref.store %max_v, %out[%n, %c, %oh, %ow] : memref<32x3x112x112xf32>
          }
        }
      }
    }
    return %out : memref<32x3x112x112xf32>
  }
}
