...
CASE_PATH = 'benchmark/choreo/gelu/6_dynamic_128xCx112x112.co'
CASE_NAME = '6_dynamic_128xCx112x112'
CATEGORY = 'gelu'
SYMBOL_DEFAULTS = {'C': 16}
INPUT_TOKENS = ['128xCx112x112']
OUTPUT_TOKEN = '128xCx112x112'
BASE_NOTES = ''
...
if triton is not None and tl is not None:
    ...
    def op_kernel(x_ptr, y_ptr, numel, block_size: tl.constexpr):
        ...
        out = values * tl.sigmoid(2.0 * 0.7978845608028654 * (values + 0.044715 * values * values * values))
...
def describe_case(env: dict[str, int]) -> dict[str, object]:
    ...
    return {
        ...
        "notes": merge_notes(BASE_NOTES, "standalone unary kernel", f"op=gelu", f"block_size={BLOCK_SIZE}"),
    }
