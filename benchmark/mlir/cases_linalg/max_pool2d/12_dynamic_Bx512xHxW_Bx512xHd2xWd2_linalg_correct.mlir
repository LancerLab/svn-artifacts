module {
  func.func @f_12_dynamic_Bx512xHxW_Bx512xHd2xWd2_correct(%input: tensor<?x512x?x?xf32>) -> tensor<?x512x?x?xf32> {
    ...
    %init = tensor.empty(%od0, %od2, %od3) : tensor<?x512x?x?xf32>
    %fill = linalg.fill ins(%neg_inf : f32) outs(%init : tensor<?x512x?x?xf32>) -> tensor<?x512x?x?xf32>
    ...
    %r = linalg.pooling_nchw_max {dilations = dense<1> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>}
         ...
         outs(%fill : tensor<?x512x?x?xf32>) -> tensor<?x512x?x?xf32>
    return %r : tensor<?x512x?x?xf32>
  }
}
