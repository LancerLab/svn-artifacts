module {
  func.func @f_9_dynamic_64x128xHxW_64x128xHd2xWd2_mismatch(%input: tensor<64x128x?x?xf32>) -> tensor<64x127x?x?xf32> {
    ...
    %init = tensor.empty(%od2, %od3) : tensor<64x127x?x?xf32>
    %fill = linalg.fill ins(%neg_inf : f32) outs(%init : tensor<64x127x?x?xf32>) -> tensor<64x127x?x?xf32>
    ...
    %r = linalg.pooling_nchw_max {dilations = dense<1> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>}
         ...
         outs(%fill : tensor<64x127x?x?xf32>) -> tensor<64x127x?x?xf32>
    return %r : tensor<64x127x?x?xf32>
  }
}
