module {
  func.func @f_16_general_64x256_20000x512_64x256x512(%indices: tensor<64x256xi64>, %table: tensor<20000x512xf32>) -> tensor<64x256x512xf32> {
    ...
    %out_init = linalg.fill ins(%zero_emb : f32) outs(%out : tensor<64x256x512xf32>) -> tensor<64x256x512xf32>
    %result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>],
      iterator_types = ["parallel", "parallel", "parallel"]
    } ins(%indices : tensor<64x256xi64>) outs(%out_init : tensor<64x256x512xf32>) {
    ^bb0(%raw_idx: i64, %init: f32):
      %ei0 = linalg.index 2 : index
      %row = arith.index_cast %raw_idx : i64 to index
      %val = tensor.extract %table[%row, %ei0] : tensor<20000x512xf32>
