module {
  func.func @f_11_dynamic_32xSx768_768_768(%input: tensor<32x?x768xf32>, %gamma: tensor<768xf32>, %beta: tensor<768xf32>) -> tensor<32x?x768xf32> {
    ...
    %gamma_d0 = tensor.dim %gamma, %c0 : tensor<768xf32>
    %beta_d0 = tensor.dim %beta, %c0 : tensor<768xf32>
    %out = tensor.empty(%input_d1) : tensor<32x?x768xf32>
    %sum_t = tensor.empty(%input_d1) : tensor<32x?xf32>
    %sumsq_t = tensor.empty(%input_d1) : tensor<32x?xf32>
    %zero_f   = arith.constant 0.0 : f32
    %sum_init = linalg.fill ins(%zero_f : f32) outs(%sum_t : tensor<32x?xf32>) -> tensor<32x?xf32>
    %sumsq_init = linalg.fill ins(%zero_f : f32) outs(%sumsq_t : tensor<32x?xf32>) -> tensor<32x?xf32>
    %nsz = arith.constant 768.0 : f32
    ...
    } ins(%input : tensor<32x?x768xf32>) outs(%sum_init, %sumsq_init : tensor<32x?xf32>, tensor<32x?xf32>) {
    ...
    } -> (tensor<32x?xf32>, tensor<32x?xf32>)
    %eps = arith.constant 1.0e-05 : f32
    %mean_t = tensor.empty(%input_d1) : tensor<32x?xf32>
    %istd_t = tensor.empty(%input_d1) : tensor<32x?xf32>
    %mean_dummy = linalg.fill ins(%zero_f : f32) outs(%mean_t : tensor<32x?xf32>) -> tensor<32x?xf32>
    %istd_dummy = linalg.fill ins(%zero_f : f32) outs(%istd_t : tensor<32x?xf32>) -> tensor<32x?xf32>
    ...
    } ins(%sum_result, %sumsq_result : tensor<32x?xf32>, tensor<32x?xf32>) outs(%mean_dummy, %istd_dummy : tensor<32x?xf32>, tensor<32x?xf32>) {
    ...
    } ins(%input, %mean_result, %istd_result, %gamma, %beta : tensor<32x?x768xf32>, tensor<32x?xf32>, tensor<32x?xf32>, tensor<768xf32>, tensor<768xf32>) outs(%out : tensor<32x?x768xf32>) {
