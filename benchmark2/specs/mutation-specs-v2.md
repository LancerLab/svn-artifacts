...
| M1.19 | index-carrier overflow (adopted; realization **b**, narrow-carrier — 32-bit carrier cannot name a legal index) | ✔ |
| M1.20 | `view` / `subspan` offset, stride and rank arity — `StaticFail`-only, so a **symbolic** offset is neither refused nor checked | ✔ |
| M1.21 | `tileat`/`at` index vs. the tiled extent, and `step`/`stride` tail on a non-divisible extent | ✔ |
...
## §2 M2 — shape-compatibility (20 specs)
...
| M2.15 | `pad_low` ↔ `pad_high` **swapped** (total length preserved, placement differs) — the check is a *sum*, so it is blind. **The M2 dual of M2.10** | ✔ |
| M2.16 | `span_as` preserving `ElementCount()` with a different rank/split (the check compares *count*, not shape) | ✔ |
| M2.17 | `span_as` / `reshape` on **runtime-shaped** data — the size check is skipped entirely | ✔ |
| M2.18 | `reshape` on a **non-contiguous** span (warning-only path) | ✔ |
| M2.19 | DMA extent mismatch where ≥ 1 extent is **symbolic** — the hard error is escaped | ✔ |
| M2.21 | broadcast of an MSB dimension whose extent is neither 1 nor equal (rank-unequal path compares trailing dims only) | ✔ |

M2.15–M2.19 and M2.21 are the v2.1 verified additions (§9.6.2). M2.16/M2.17/M2.19 share
one root cause and together with M2.10/M2.13 form the **view/metadata family** — the top
triggering axis in both empirical corpora. M2.20/M2.22 remain provisional (§9.6.3).
...
## §3 M3 — hardware-constraint (16 specs, redesigned)
...
| M3.13 | swizzle width ↔ box inner dim ↔ shared alignment, narrowed — the injected state must **survive the compiler's own repair** (`cute_codegen.cpp:10271`) | K2 encoding | ✔ |
| M3.14 | **linear `.copy` with a dimension ≥ 2²⁴ — the check is absent** (`gpu_adapt.hpp:320` `// omitted`) | **A, unchecked** | ✔ |
| M3.15 | **`.pad` with a dimension ≥ 2²⁴ — the pad path never calls `CheckDimSize`** | **A, unchecked** | ✔ |
| M3.16 | TMA box inner alignment with a **symbolic** leading dim (`gpu_adapt.hpp:640` `// TODO` → no assessment) | K2, unchecked | ✔ |
...
(`dma.transp` permutation with a repeated dim) · M3.17 (sub-byte/bit-typed element
geometry: `F4_E2M1`, `F6_*`, `BIN1`, `U1`) · M3.18 (`promote<T>` geometry) · M3.19
(`multicast` / `sparse<m,n>` metadata vs. operand) · M3.20 (MMA atom **M/N** divisibility,
distinct from the atom-K that M3.1 covers) · M3.21 (`inttuple`/`chunkat` index-count ≠
descriptor rank) · plus 2 M2 confirmation items.

Verification commands: `mutation-expansion-m1m2m3.md` §7.
...
| # | Defect | Evidence |
|---|---|---|
| F1 | Linear `.copy` has no dimension-size check | `gpu_adapt.hpp:320` `// omitted` |
| F2 | The `.pad` path never checks dimension sizes | `gpu_adapt.hpp:360-450` |
| F3 | TMA box alignment skipped for a symbolic leading dim | `gpu_adapt.hpp:640` `// TODO` |
| F4 | DMA extent mismatch not an error when either extent is symbolic | `semacheck.cpp:1140-1145` |
| F5 | `span_as` on runtime-shaped data escapes the size check | `semacheck.cpp:946-950` |

F1/F2 are the strongest (an explicit `// omitted`), with F3 (`// TODO`) close behind.
Always additionally in scope: the `(int)` index-carrier truncation at
`cute_codegen.cpp:1757` (the adopted M1.19).
