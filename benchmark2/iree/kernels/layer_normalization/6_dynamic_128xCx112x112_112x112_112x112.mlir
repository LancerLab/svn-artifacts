module {
  func.func @f_6_dynamic_128xCx112x112_112x112_112x112(%input: tensor<128x?x112x112xf32>, %gamma: tensor<112x112xf32>, %beta: tensor<112x112xf32>) -> tensor<128x?x112x112xf32> {
    ...
    %sum_t = tensor.empty(%input_d1) : tensor<128x?xf32>
    %sumsq_t = tensor.empty(%input_d1) : tensor<128x?xf32>
    %zero_f   = arith.constant 0.0 : f32
    %sum_init = linalg.fill ins(%zero_f : f32) outs(%sum_t : tensor<128x?xf32>) -> tensor<128x?xf32>
    %sumsq_init = linalg.fill ins(%zero_f : f32) outs(%sumsq_t : tensor<128x?xf32>) -> tensor<128x?xf32>
    ...
    } ins(%input : tensor<128x?x112x112xf32>) outs(%sum_init, %sumsq_init : tensor<128x?xf32>, tensor<128x?xf32>) {
    ...
    } -> (tensor<128x?xf32>, tensor<128x?xf32>)
    %eps = arith.constant 1.0e-05 : f32
    %mean_t = tensor.empty(%input_d1) : tensor<128x?xf32>
    %istd_t = tensor.empty(%input_d1) : tensor<128x?xf32>
    %mean_dummy = linalg.fill ins(%zero_f : f32) outs(%mean_t : tensor<128x?xf32>) -> tensor<128x?xf32>
    %istd_dummy = linalg.fill ins(%zero_f : f32) outs(%istd_t : tensor<128x?xf32>) -> tensor<128x?xf32>
    ...
    } ins(%sum_result, %sumsq_result : tensor<128x?xf32>, tensor<128x?xf32>) outs(%mean_dummy, %istd_dummy : tensor<128x?xf32>, tensor<128x?xf32>) {
    ...
    } ins(%input, %mean_result, %istd_result, %gamma, %beta : tensor<128x?x112x112xf32>, tensor<128x?xf32>, tensor<128x?xf32>, tensor<112x112xf32>, tensor<112x112xf32>) outs(%out : tensor<128x?x112x112xf32>) {
