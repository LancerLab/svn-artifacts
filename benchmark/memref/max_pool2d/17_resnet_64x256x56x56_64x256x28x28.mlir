module @f_17_resnet_64x256x56x56_64x256x28x28 {
  func.func @f_17_resnet_64x256x56x56_64x256x28x28(%input: memref<64x256x56x56xf32>) -> memref<64x256x28x28xf32> {
    ...
    %out = memref.alloc() : memref<64x256x28x28xf32>
          ...
          scf.for %ow = %c0 to %out_d3 step %c1 {
            ...
            %max_v = scf.for %kh = %c0 to %kH_sz step %c1 iter_args(%mx_h = %neg_inf) -> (f32) {
              %max_v2 = scf.for %kw = %c0 to %kW_sz step %c1 iter_args(%mx_w = %mx_h) -> (f32) {
                ...
                %pv   = memref.load %input[%n, %c, %ih, %iw] : memref<64x256x56x56xf32>
            ...
            memref.store %max_v, %out[%n, %c, %oh, %ow] : memref<64x256x28x28xf32>
          }
        }
      }
    }
    return %out : memref<64x256x28x28xf32>
  }
}
