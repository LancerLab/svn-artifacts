module @f_15_gpt_16x1024x4096_4096_4096_16x1024x4096 attributes {hal.device.targets = [#hal.device.target<"cuda", {executable_targets = [#hal.executable.target<"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}>], legacy_sync}>]} {
  hal.executable private @f_15_gpt_16x1024x4096_4096_4096_16x1024x4096_dispatch_0 {
    hal.executable.variant public @cuda_nvptx_fb, target = <"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}> {
      hal.executable.export public @f_15_gpt_16x1024x4096_4096_4096_16x1024x4096_dispatch_0_generic_16x1024x4096_f32 ordinal(0) layout(#hal.pipeline.layout<push_constants = 0, sets = [<0, bindings = [<0, storage_buffer, ReadOnly>, <1, storage_buffer, ReadOnly>, <2, storage_buffer, ReadOnly>, <3, storage_buffer, ReadOnly>, <4, storage_buffer>]>]>) {
      ^bb0(%arg0: !hal.device):
        %x, %y, %z = flow.dispatch.workgroup_count_from_slice 
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @f_15_gpt_16x1024x4096_4096_4096_16x1024x4096_dispatch_0_generic_16x1024x4096_f32() {
          ...
          %1 = hal.interface.binding.subspan set(0) binding(1) type(storage_buffer) alignment(64) offset(%c0) flags(ReadOnly) : !flow.dispatch.tensor<readonly:tensor<4096xf32>>
          ...
          %6 = flow.dispatch.tensor.load %1, offsets = [0], sizes = [4096], strides = [1] : !flow.dispatch.tensor<readonly:tensor<4096xf32>> -> tensor<4096xf32>
          ...
          %10 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d2)>, affine_map<(d0, d1, d2) -> (d2)>, affine_map<(d0, d1, d2) -> (d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%5, %6, %7, %8 : tensor<16x1024x4096xf32>, tensor<4096xf32>, tensor<4096xf32>, tensor<4096xf32>) outs(%9 : tensor<16x1024x4096xf32>) {
          ...
          flow.dispatch.tensor.store %10, %4, offsets = [0, 0, 0], sizes = [16, 1024, 4096], strides = [1, 1, 1] : tensor<16x1024x4096xf32> -> !flow.dispatch.tensor<writeonly:tensor<16x1024x4096xf32>>
          return
        }
      }
    }
  }
  func.func @f_15_gpt_16x1024x4096_4096_4096_16x1024x4096(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    ...
    %result, %result_timepoint = stream.resource.alloca uninitialized : !stream.resource<transient>{%c16384} => !stream.timepoint
    %4 = stream.cmd.execute await(%result_timepoint) => with(%0 as %arg3: !stream.resource<external>{%c268435456}, %1 as %arg4: !stream.resource<external>{%c16384}, %2 as %arg5: !stream.resource<external>{%c16384}, %3 as %arg6: !stream.resource<external>{%c268435456}, %result as %arg7: !stream.resource<transient>{%c16384}) {
      stream.cmd.fill %c1065353258_i32, %arg7[%c0 for %c16384] : i32 -> !stream.resource<transient>{%c16384}
      stream.cmd.dispatch @f_15_gpt_16x1024x4096_4096_4096_16x1024x4096_dispatch_0::@cuda_nvptx_fb::@f_15_gpt_16x1024x4096_4096_4096_16x1024x4096_dispatch_0_generic_16x1024x4096_f32 {
        ro %arg3[%c0 for %c268435456] : !stream.resource<external>{%c268435456},
        ro %arg4[%c0 for %c16384] : !stream.resource<external>{%c16384},
        ro %arg7[%c0 for %c16384] : !stream.resource<transient>{%c16384},
    ...
    %5 = stream.resource.dealloca await(%4) => %result : !stream.resource<transient>{%c16384} => !stream.timepoint
