module {
  func.func @f_5_dynamic_Nx1280xMxK_MxK_MxK(%input: tensor<?x1280x?x?xf32>, %gamma: tensor<?x?xf32>, %beta: tensor<?x?xf32>) -> tensor<?x1280x?x?xf32> {
    ...
    %sum_t = tensor.empty(%input_d0) : tensor<?x1280xf32>
    %sumsq_t = tensor.empty(%input_d0) : tensor<?x1280xf32>
    %zero_f   = arith.constant 0.0 : f32
    %sum_init = linalg.fill ins(%zero_f : f32) outs(%sum_t : tensor<?x1280xf32>) -> tensor<?x1280xf32>
    %sumsq_init = linalg.fill ins(%zero_f : f32) outs(%sumsq_t : tensor<?x1280xf32>) -> tensor<?x1280xf32>
    ...
    } ins(%input : tensor<?x1280x?x?xf32>) outs(%sum_init, %sumsq_init : tensor<?x1280xf32>, tensor<?x1280xf32>) {
    ...
    } -> (tensor<?x1280xf32>, tensor<?x1280xf32>)
    %eps = arith.constant 1.0e-05 : f32
    %mean_t = tensor.empty(%input_d0) : tensor<?x1280xf32>
    %istd_t = tensor.empty(%input_d0) : tensor<?x1280xf32>
    %mean_dummy = linalg.fill ins(%zero_f : f32) outs(%mean_t : tensor<?x1280xf32>) -> tensor<?x1280xf32>
    %istd_dummy = linalg.fill ins(%zero_f : f32) outs(%istd_t : tensor<?x1280xf32>) -> tensor<?x1280xf32>
    ...
    } ins(%sum_result, %sumsq_result : tensor<?x1280xf32>, tensor<?x1280xf32>) outs(%mean_dummy, %istd_dummy : tensor<?x1280xf32>, tensor<?x1280xf32>) {
    ...
    } ins(%input, %mean_result, %istd_result, %gamma, %beta : tensor<?x1280x?x?xf32>, tensor<?x1280xf32>, tensor<?x1280xf32>, tensor<?x?xf32>, tensor<?x?xf32>) outs(%out : tensor<?x1280x?x?xf32>) {
