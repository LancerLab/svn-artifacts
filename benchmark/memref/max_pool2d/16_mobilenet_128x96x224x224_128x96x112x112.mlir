module @f_16_mobilenet_128x96x224x224_128x96x112x112 {
  func.func @f_16_mobilenet_128x96x224x224_128x96x112x112(%input: memref<128x96x224x224xf32>) -> memref<128x96x112x112xf32> {
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c2 = arith.constant 2 : index
    %c3 = arith.constant 3 : index
    %input_d0 = memref.dim %input, %c0 : memref<128x96x224x224xf32>
    %input_d1 = memref.dim %input, %c1 : memref<128x96x224x224xf32>
    %input_d2 = memref.dim %input, %c2 : memref<128x96x224x224xf32>
    %input_d3 = memref.dim %input, %c3 : memref<128x96x224x224xf32>
    ...
    %out_d2 = arith.constant 112 : index
    %out_d3 = arith.constant 112 : index
          ...
          scf.for %ow = %c0 to %out_d3 step %c1 {
            ...
            %max_v = scf.for %kh = %c0 to %kH_sz step %c1 iter_args(%mx_h = %neg_inf) -> (f32) {
              %max_v2 = scf.for %kw = %c0 to %kW_sz step %c1 iter_args(%mx_w = %mx_h) -> (f32) {
                ...
                %pv   = memref.load %input[%n, %c, %ih, %iw] : memref<128x96x224x224xf32>
            ...
            memref.store %max_v, %out[%n, %c, %oh, %ow] : memref<128x96x112x112xf32>
          }
        }
      }
    }
