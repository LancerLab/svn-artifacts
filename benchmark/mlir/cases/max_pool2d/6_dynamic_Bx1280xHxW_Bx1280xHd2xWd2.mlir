module {
  func.func @f_6_dynamic_Bx1280xHxW_Bx1280xHd2xWd2(%input: tensor<?x1280x?x?xf32>) -> tensor<?x1280x?x?xf32> {
    ...
    %t_res_pn = scf.for %pn = %c0 to %input_d0 step %c1 iter_args(%t_mp_pn = %out) -> (tensor<?x1280x?x?xf32>) {
    %t_res_pc = scf.for %pc = %c0 to %input_d1 step %c1 iter_args(%t_mp_pc = %t_mp_pn) -> (tensor<?x1280x?x?xf32>) {
    %t_res_poh = scf.for %poh = %c0 to %input_d2 step %c1 iter_args(%t_mp_poh = %t_mp_pc) -> (tensor<?x1280x?x?xf32>) {
    %t_res_pow = scf.for %pow = %c0 to %input_d3 step %c1 iter_args(%t_mp_pow = %t_mp_poh) -> (tensor<?x1280x?x?xf32>) {
    ...
    %pv    = tensor.extract %input[%pn, %pc, %ih, %iw] : tensor<?x1280x?x?xf32>
    ...
    %t_new = tensor.insert %pool into %t_mp_pow[%pn, %pc, %poh, %pow] : tensor<?x1280x?x?xf32>
    scf.yield %t_new : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_pow : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_poh : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_pc : tensor<?x1280x?x?xf32>
    }
    return %t_res_pn : tensor<?x1280x?x?xf32>
  }
}
