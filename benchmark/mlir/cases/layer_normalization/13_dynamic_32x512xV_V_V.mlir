module {
  func.func @f_13_dynamic_32x512xV_V_V(%input: tensor<32x512x?xf32>, %gamma: tensor<?xf32>, %beta: tensor<?xf32>) -> tensor<32x512x?xf32> {
    ...
    %t_res_ln_b0 = scf.for %ln_b0 = %c0 to %input_d0 step %c1 iter_args(%t_ln_ln_b0 = %out) -> (tensor<32x512x?xf32>) {
    %t_res_ln_b1 = scf.for %ln_b1 = %c0 to %input_d1 step %c1 iter_args(%t_ln_ln_b1 = %t_ln_ln_b0) -> (tensor<32x512x?xf32>) {
    ...
    %lnv1 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0] : tensor<32x512x?xf32>
    %ns1  = arith.addf %ln_sa0, %lnv1 : f32
    scf.yield %ns1 : f32
    }
    %ln_sumsq = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%ln_ssa0 = %zero_sf) -> (f32) {
    %lnv2 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0] : tensor<32x512x?xf32>
    ...
    %t_res_ln_n0 = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%t_ln_n0 = %t_ln_ln_b1) -> (tensor<32x512x?xf32>) {
    %lnv3   = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0] : tensor<32x512x?xf32>
    %gv     = tensor.extract %gamma[%ln_n0] : tensor<?xf32>
    %bv     = tensor.extract %beta[%ln_n0]  : tensor<?xf32>
    ...
    %t_ins_ln = tensor.insert %res into %t_ln_n0[%ln_b0, %ln_b1, %ln_n0] : tensor<32x512x?xf32>
    scf.yield %t_ins_ln : tensor<32x512x?xf32>
    }
    scf.yield %t_res_ln_n0 : tensor<32x512x?xf32>
    }
    scf.yield %t_res_ln_b1 : tensor<32x512x?xf32>
    }
    return %t_res_ln_b0 : tensor<32x512x?xf32>
  }
}
