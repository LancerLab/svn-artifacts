// sigmoid.cu — y[i] = 1 / (1 + exp(-a[i])).
#include <cute/tensor.hpp>
#include "common.cuh"
using namespace cute;

__global__ void k_sigmoid(const float* __restrict__ A, float* __restrict__ C,
                          long n) {
...
int main(int argc, char** argv) {
  ...
  k_sigmoid<<<(unsigned)grid, threads, smem>>>(A, C, n);
