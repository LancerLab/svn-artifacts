module {
  func.func @f_17_static_8x256x64x64_21x256x1x1_8x21x64x64_1_0_1(%input: tensor<8x256x64x64xf32>, %filter: tensor<21x256x1x1xf32>) -> tensor<8x21x64x64xf32> {
    ...
    %out_sz2 = arith.constant 64 : index
    %out_sz3 = arith.constant 64 : index
    %t_res_n = scf.for %n = %c0 to %input_d0 step %c1 iter_args(%t_cv_n = %out) -> (tensor<8x21x64x64xf32>) {
    %t_res_f = scf.for %f = %c0 to %filter_d0 step %c1 iter_args(%t_cv_f = %t_cv_n) -> (tensor<8x21x64x64xf32>) {
    %t_res_oh = scf.for %oh = %c0 to %out_sz2 step %c1 iter_args(%t_cv_oh = %t_cv_f) -> (tensor<8x21x64x64xf32>) {
    %t_res_ow = scf.for %ow = %c0 to %out_sz3 step %c1 iter_args(%t_cv_ow = %t_cv_oh) -> (tensor<8x21x64x64xf32>) {
    ...
    %xv   = tensor.extract %input[%n, %c, %ih, %iw]  : tensor<8x256x64x64xf32>
    %wv   = tensor.extract %filter[%f, %c, %kh, %kw] : tensor<21x256x1x1xf32>
    ...
    %t_new = tensor.insert %acc into %t_cv_ow[%n, %f, %oh, %ow] : tensor<8x21x64x64xf32>
    scf.yield %t_new : tensor<8x21x64x64xf32>
    }
    scf.yield %t_res_ow : tensor<8x21x64x64xf32>
    }
    scf.yield %t_res_oh : tensor<8x21x64x64xf32>
    }
    scf.yield %t_res_f : tensor<8x21x64x64xf32>
    }
    return %t_res_n : tensor<8x21x64x64xf32>
  }
}
