module {
  func.func @f_8_dynamic_16x1024xD_16x1024xD_16x1024xD(%lhs: tensor<16x1024x?xf32>, %rhs: tensor<16x1024x?xf32>) -> tensor<16x1024x?xf32> {
    ...
    %t_res_ei0 = scf.for %ei0 = %c0 to %lhs_d0 step %c1 iter_args(%t_ea_ei0 = %out) -> (tensor<16x1024x?xf32>) {
    %t_res_ei1 = scf.for %ei1 = %c0 to %lhs_d1 step %c1 iter_args(%t_ea_ei1 = %t_ea_ei0) -> (tensor<16x1024x?xf32>) {
    %t_res_ei2 = scf.for %ei2 = %c0 to %lhs_d2 step %c1 iter_args(%t_ea_ei2 = %t_ea_ei1) -> (tensor<16x1024x?xf32>) {
    %va = tensor.extract %lhs[%ei0, %ei1, %ei2] : tensor<16x1024x?xf32>
    %vb = tensor.extract %rhs[%ei0, %ei1, %ei2] : tensor<16x1024x?xf32>
    %rs = arith.addf %va, %vb : f32
    %t_ins = tensor.insert %rs into %t_ea_ei2[%ei0, %ei1, %ei2] : tensor<16x1024x?xf32>
    scf.yield %t_ins : tensor<16x1024x?xf32>
    }
    scf.yield %t_res_ei2 : tensor<16x1024x?xf32>
    }
    scf.yield %t_res_ei1 : tensor<16x1024x?xf32>
    }
    return %t_res_ei0 : tensor<16x1024x?xf32>
  }
}
