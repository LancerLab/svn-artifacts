module {
  func.func @f_5_dynamic_Nx1280xMxK_MxK_MxK(%input: tensor<?x1280x?x?xf32>, %gamma: tensor<?x?xf32>, %beta: tensor<?x?xf32>) -> tensor<?x1280x?x?xf32> {
    ...
    %t_res_ln_b0 = scf.for %ln_b0 = %c0 to %input_d0 step %c1 iter_args(%t_ln_ln_b0 = %out) -> (tensor<?x1280x?x?xf32>) {
    %t_res_ln_b1 = scf.for %ln_b1 = %c0 to %input_d1 step %c1 iter_args(%t_ln_ln_b1 = %t_ln_ln_b0) -> (tensor<?x1280x?x?xf32>) {
    ...
    %lnv1 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x1280x?x?xf32>
    ...
    %lnv2 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x1280x?x?xf32>
    ...
    %t_res_ln_n0 = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%t_ln_n0 = %t_ln_ln_b1) -> (tensor<?x1280x?x?xf32>) {
    %t_res_ln_n1 = scf.for %ln_n1 = %c0 to %input_d3 step %c1 iter_args(%t_ln_n1 = %t_ln_n0) -> (tensor<?x1280x?x?xf32>) {
    %lnv3   = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x1280x?x?xf32>
    ...
    %t_ins_ln = tensor.insert %res into %t_ln_n1[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x1280x?x?xf32>
    scf.yield %t_ins_ln : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_ln_n1 : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_ln_n0 : tensor<?x1280x?x?xf32>
    }
    scf.yield %t_res_ln_b1 : tensor<?x1280x?x?xf32>
    }
    return %t_res_ln_b0 : tensor<?x1280x?x?xf32>
  }
}
