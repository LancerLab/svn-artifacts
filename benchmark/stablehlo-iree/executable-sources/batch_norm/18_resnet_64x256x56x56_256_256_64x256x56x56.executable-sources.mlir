module @f_18_resnet_64x256x56x56_256_256_64x256x56x56 attributes {hal.device.targets = [#hal.device.target<"cuda", {executable_targets = [#hal.executable.target<"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}>], legacy_sync}>]} {
  hal.executable private @f_18_resnet_64x256x56x56_256_256_64x256x56x56_dispatch_0 {
    hal.executable.variant public @cuda_nvptx_fb, target = <"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}> {
      hal.executable.export public @f_18_resnet_64x256x56x56_256_256_64x256x56x56_dispatch_0_generic_64x256x56x56_f32 ordinal(0) layout(#hal.pipeline.layout<push_constants = 0, sets = [<0, bindings = [<0, storage_buffer, ReadOnly>, <1, storage_buffer, ReadOnly>, <2, storage_buffer, ReadOnly>, <3, storage_buffer, ReadOnly>, <4, storage_buffer>]>]>) {
      ^bb0(%arg0: !hal.device):
        %x, %y, %z = flow.dispatch.workgroup_count_from_slice 
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @f_18_resnet_64x256x56x56_256_256_64x256x56x56_dispatch_0_generic_64x256x56x56_f32() {
          ...
          %4 = hal.interface.binding.subspan set(0) binding(4) type(storage_buffer) alignment(64) offset(%c0) : !flow.dispatch.tensor<writeonly:tensor<64x256x56x56xf32>>
          %5 = flow.dispatch.tensor.load %0, offsets = [0, 0, 0, 0], sizes = [64, 256, 56, 56], strides = [1, 1, 1, 1] : !flow.dispatch.tensor<readonly:tensor<64x256x56x56xf32>> -> tensor<64x256x56x56xf32>
          ...
          %9 = tensor.empty() : tensor<64x256x56x56xf32>
          %10 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d1)>, affine_map<(d0, d1, d2, d3) -> (d1)>, affine_map<(d0, d1, d2, d3) -> (d1)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%5, %6, %7, %8 : tensor<64x256x56x56xf32>, tensor<256xf32>, tensor<256xf32>, tensor<256xf32>) outs(%9 : tensor<64x256x56x56xf32>) {
          ...
          flow.dispatch.tensor.store %10, %4, offsets = [0, 0, 0, 0], sizes = [64, 256, 56, 56], strides = [1, 1, 1, 1] : tensor<64x256x56x56xf32> -> !flow.dispatch.tensor<writeonly:tensor<64x256x56x56xf32>>
          return
        }
      }
    }
  }
  func.func @f_18_resnet_64x256x56x56_256_256_64x256x56x56(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view, %arg2: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    ...
    %4 = stream.cmd.execute await(%result_timepoint) => with(%0 as %arg3: !stream.resource<external>{%c205520896}, %1 as %arg4: !stream.resource<external>{%c1024}, %2 as %arg5: !stream.resource<external>{%c1024}, %3 as %arg6: !stream.resource<external>{%c205520896}, %result as %arg7: !stream.resource<transient>{%c1024}) {
      stream.cmd.fill %c1065353258_i32, %arg7[%c0 for %c1024] : i32 -> !stream.resource<transient>{%c1024}
      stream.cmd.dispatch @f_18_resnet_64x256x56x56_256_256_64x256x56x56_dispatch_0::@cuda_nvptx_fb::@f_18_resnet_64x256x56x56_256_256_64x256x56x56_dispatch_0_generic_64x256x56x56_f32 {
