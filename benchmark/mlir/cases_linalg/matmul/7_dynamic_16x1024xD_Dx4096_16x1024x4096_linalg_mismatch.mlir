...
module {
  func.func @f_7_dynamic_16x1024xD_Dx4096_16x1024x4096_mismatch(%a: tensor<16x1024x?xf32>, %b: tensor<?x4096xf32>) -> tensor<16x1024x4096xf32> {
    %cst = arith.constant 0.0 : f32
    %init = tensor.empty() : tensor<16x1024x4096xf32>
    %fill = linalg.fill ins(%cst : f32) outs(%init : tensor<16x1024x4096xf32>) -> tensor<16x1024x4096xf32>
    %r = linalg.generic {
      indexing_maps = [#map_lhs, #map_rhs, #map_out],
      iterator_types = ["parallel", "parallel", "parallel", "reduction"]
    } ins(%a, %b : tensor<16x1024x?xf32>, tensor<?x4096xf32>)
      outs(%fill : tensor<16x1024x4096xf32>) {
