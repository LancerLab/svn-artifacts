module {
  func.func @f_20_vit_32x197x3072_32x197x3072(%input: tensor<32x197x3072xf32>) -> tensor<32x197x3072xf32> {
    ...
    } ins(%input : tensor<32x197x3072xf32>) outs(%max_init : tensor<32x197xf32>) {
    ^bb0(%in: f32, %cur: f32):
    ...
    %sum_result = linalg.generic {
      ...
      iterator_types = ["parallel", "parallel", "reduction"]
    } ins(%input, %max_result : tensor<32x197x3072xf32>, tensor<32x197xf32>) outs(%sum_init : tensor<32x197xf32>) {
    ...
    } ins(%input, %max_result, %sum_result : tensor<32x197x3072xf32>, tensor<32x197xf32>, tensor<32x197xf32>) outs(%out : tensor<32x197x3072xf32>) {
