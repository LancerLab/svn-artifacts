module {
  func.func @f_18_unet_16x64x128x128_128x64x3x3_16x128x128x128_S_P_D(%input: tensor<16x64x128x128xf32>, %filter: tensor<128x64x3x3xf32>) -> tensor<16x128x128x128xf32> {
    ...
    %out_sz2 = arith.constant 128 : index
    %out_sz3 = arith.constant 128 : index
    %t_res_n = scf.for %n = %c0 to %input_d0 step %c1 iter_args(%t_cv_n = %out) -> (tensor<16x128x128x128xf32>) {
    %t_res_f = scf.for %f = %c0 to %filter_d0 step %c1 iter_args(%t_cv_f = %t_cv_n) -> (tensor<16x128x128x128xf32>) {
    %t_res_oh = scf.for %oh = %c0 to %out_sz2 step %c1 iter_args(%t_cv_oh = %t_cv_f) -> (tensor<16x128x128x128xf32>) {
    %t_res_ow = scf.for %ow = %c0 to %out_sz3 step %c1 iter_args(%t_cv_ow = %t_cv_oh) -> (tensor<16x128x128x128xf32>) {
    ...
    %xv   = tensor.extract %input[%n, %c, %ih, %iw]  : tensor<16x64x128x128xf32>
    ...
    %t_new = tensor.insert %acc into %t_cv_ow[%n, %f, %oh, %ow] : tensor<16x128x128x128xf32>
    scf.yield %t_new : tensor<16x128x128x128xf32>
    }
    scf.yield %t_res_ow : tensor<16x128x128x128xf32>
    }
    scf.yield %t_res_oh : tensor<16x128x128x128xf32>
    }
    scf.yield %t_res_f : tensor<16x128x128x128xf32>
    }
    return %t_res_n : tensor<16x128x128x128xf32>
  }
}
