module @f_2_cnn_128x128x28x28_128x28x28x128 {
  func.func @f_2_cnn_128x128x28x28_128x28x28x128(%input: memref<128x128x28x28xf32>) -> memref<128x28x28x128xf32> {
    ...
    %out = memref.alloc() : memref<128x28x28x128xf32>
      ...
      scf.for %pi1 = %c0 to %input_d2 step %c1 {
        scf.for %pi2 = %c0 to %input_d3 step %c1 {
          scf.for %pi3 = %c0 to %input_d1 step %c1 {
            %tv    = memref.load %input[%pi0, %pi3, %pi1, %pi2] : memref<128x128x28x28xf32>
            memref.store %tv, %out[%pi0, %pi1, %pi2, %pi3] : memref<128x28x28x128xf32>
          }
        }
      }
    }
    return %out : memref<128x28x28x128xf32>
  }
}
