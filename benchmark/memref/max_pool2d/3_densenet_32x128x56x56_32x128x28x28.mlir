module @f_3_densenet_32x128x56x56_32x128x28x28 {
  func.func @f_3_densenet_32x128x56x56_32x128x28x28(%input: memref<32x128x56x56xf32>) -> memref<32x128x28x28xf32> {
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c2 = arith.constant 2 : index
    %c3 = arith.constant 3 : index
    %input_d0 = memref.dim %input, %c0 : memref<32x128x56x56xf32>
    %input_d1 = memref.dim %input, %c1 : memref<32x128x56x56xf32>
    %input_d2 = memref.dim %input, %c2 : memref<32x128x56x56xf32>
    %input_d3 = memref.dim %input, %c3 : memref<32x128x56x56xf32>
    %out = memref.alloc() : memref<32x128x28x28xf32>
    %kH_sz  = arith.constant 2 : index
    %kW_sz  = arith.constant 2 : index
    %stride = arith.constant 2 : index
    %out_d2 = arith.constant 28 : index
    %out_d3 = arith.constant 28 : index
          ...
          scf.for %ow = %c0 to %out_d3 step %c1 {
            ...
            %max_v = scf.for %kh = %c0 to %kH_sz step %c1 iter_args(%mx_h = %neg_inf) -> (f32) {
              %max_v2 = scf.for %kw = %c0 to %kW_sz step %c1 iter_args(%mx_w = %mx_h) -> (f32) {
                ...
                %pv   = memref.load %input[%n, %c, %ih, %iw] : memref<32x128x56x56xf32>
            ...
            memref.store %max_v, %out[%n, %c, %oh, %ow] : memref<32x128x28x28xf32>
          }
        }
      }
    }
    return %out : memref<32x128x28x28xf32>
  }
}
