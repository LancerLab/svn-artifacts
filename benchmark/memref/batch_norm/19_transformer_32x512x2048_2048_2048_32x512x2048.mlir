module @f_19_transformer_32x512x2048_2048_2048_32x512x2048 {
  func.func @f_19_transformer_32x512x2048_2048_2048_32x512x2048(%input: memref<32x512x2048xf32>, %gamma: memref<2048xf32>, %beta: memref<2048xf32>) -> memref<32x512x2048xf32> {
    ...
    scf.for %bn0 = %c0 to %input_d0 step %c1 {
      scf.for %bn1 = %c0 to %input_d1 step %c1 {
        scf.for %bn2 = %c0 to %input_d2 step %c1 {
          %xv  = memref.load %input[%bn0, %bn1, %bn2] : memref<32x512x2048xf32>
          %gv  = memref.load %gamma[%bn2] : memref<2048xf32>
          %bv  = memref.load %beta[%bn2]  : memref<2048xf32>
          %scl = arith.mulf %xv, %gv : f32
          %res = arith.addf %scl, %bv : f32
          memref.store %res, %out[%bn0, %bn1, %bn2] : memref<32x512x2048xf32>
        }
      }
    }
