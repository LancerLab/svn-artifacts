module @f_2_cnn_128x128x28x28_128x128x14x14 {
  func.func @f_2_cnn_128x128x28x28_128x128x14x14(%input: memref<128x128x28x28xf32>) -> memref<128x128x14x14xf32> {
    ...
    %out = memref.alloc() : memref<128x128x14x14xf32>
          ...
          scf.for %ow = %c0 to %out_d3 step %c1 {
            ...
            %max_v = scf.for %kh = %c0 to %kH_sz step %c1 iter_args(%mx_h = %neg_inf) -> (f32) {
              %max_v2 = scf.for %kw = %c0 to %kW_sz step %c1 iter_args(%mx_w = %mx_h) -> (f32) {
                ...
                %pv   = memref.load %input[%n, %c, %ih, %iw] : memref<128x128x28x28xf32>
            ...
            memref.store %max_v, %out[%n, %c, %oh, %ow] : memref<128x128x14x14xf32>
          }
        }
      }
    }
    return %out : memref<128x128x14x14xf32>
  }
}
