...
module {
  func.func @f_17_general_32x512x512_512x512_32x512x512_mismatch(%a: tensor<32x512x512xf32>, %b: tensor<511x512xf32>) -> tensor<32x512x512xf32> {
    ...
    } ins(%a, %b : tensor<32x512x512xf32>, tensor<511x512xf32>)
      outs(%fill : tensor<32x512x512xf32>) {
    ^bb0(%in_a: f32, %in_b: f32, %acc: f32):
      %mul = arith.mulf %in_a, %in_b : f32
      %add = arith.addf %acc, %mul : f32
      linalg.yield %add : f32
    } -> tensor<32x512x512xf32>
    return %r : tensor<32x512x512xf32>
  }
}
