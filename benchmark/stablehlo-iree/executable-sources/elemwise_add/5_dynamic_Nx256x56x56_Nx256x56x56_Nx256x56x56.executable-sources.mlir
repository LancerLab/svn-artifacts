module @f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56 attributes {hal.device.targets = [#hal.device.target<"cuda", {executable_targets = [#hal.executable.target<"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}>], legacy_sync}>]} {
  hal.executable private @f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56_dispatch_0 {
    hal.executable.variant public @cuda_nvptx_fb, target = <"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}> {
      hal.executable.export public @f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56_dispatch_0_generic_Dx256x56x56_f32 ordinal(0) layout(#hal.pipeline.layout<push_constants = 2, sets = [<0, bindings = [<0, storage_buffer, ReadOnly>, <1, storage_buffer, ReadOnly>, <2, storage_buffer>]>]>) {
      ^bb0(%arg0: !hal.device, %arg1: index, %arg2: index):
        %x, %y, %z = flow.dispatch.workgroup_count_from_slice %arg1, %arg2
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56_dispatch_0_generic_Dx256x56x56_f32() {
          ...
          %8 = hal.interface.binding.subspan set(0) binding(2) type(storage_buffer) alignment(64) offset(%c0) : !flow.dispatch.tensor<writeonly:tensor<?x256x56x56xf32>>{%5}
          ...
          %11 = tensor.empty(%5) : tensor<?x256x56x56xf32>
          %12 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%9, %10 : tensor<?x256x56x56xf32>, tensor<?x256x56x56xf32>) outs(%11 : tensor<?x256x56x56xf32>) {
          ^bb0(%in: f32, %in_0: f32, %out: f32):
            %13 = arith.addf %in, %in_0 : f32
            linalg.yield %13 : f32
          } -> tensor<?x256x56x56xf32>
          flow.dispatch.tensor.store %12, %8, offsets = [0, 0, 0, 0], sizes = [%5, 256, 56, 56], strides = [1, 1, 1, 1] : tensor<?x256x56x56xf32> -> !flow.dispatch.tensor<writeonly:tensor<?x256x56x56xf32>>{%5}
          return
        }
      }
    }
  ...
  func.func @f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56(%arg0: !hal.buffer_view, %arg1: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    ...
    %9 = stream.cmd.execute with(%2 as %arg2: !stream.resource<external>{%1}, %5 as %arg3: !stream.resource<external>{%4}, %6 as %arg4: !stream.resource<external>{%1}) {
      stream.cmd.dispatch @f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56_dispatch_0::@cuda_nvptx_fb::@f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56_dispatch_0_generic_Dx256x56x56_f32[%3, %0](%7, %8 : i32, i32) {
    ...
    %11 = stream.tensor.export %10 : tensor<?x256x56x56xf32>{%0} in !stream.resource<external>{%1} -> !hal.buffer_view
