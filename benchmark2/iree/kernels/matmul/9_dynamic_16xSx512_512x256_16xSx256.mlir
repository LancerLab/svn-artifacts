module {
  func.func @f_9_dynamic_16xSx512_512x256_16xSx256(%lhs: tensor<16x?x512xf32>, %rhs: tensor<512x256xf32>) -> tensor<16x?x256xf32> {
    ...
    %filled = linalg.fill ins(%zero : f32) outs(%out : tensor<16x?x256xf32>) -> tensor<16x?x256xf32>
    %result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d3)>, affine_map<(d0, d1, d2, d3) -> (d3, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>],
      iterator_types = ["parallel", "parallel", "parallel", "reduction"]
    } ins(%lhs, %rhs : tensor<16x?x512xf32>, tensor<512x256xf32>) outs(%filled : tensor<16x?x256xf32>) {
    ^bb0(%a: f32, %b: f32, %acc: f32):
      %p = arith.mulf %a, %b : f32
      %r = arith.addf %acc, %p : f32
      linalg.yield %r : f32
    } -> tensor<16x?x256xf32>
    return %result : tensor<16x?x256xf32>
  }
}
