module {
  func.func @f_7_dynamic_32x197xD_D_D(%input: tensor<32x197x?xf32>, %gamma: tensor<?xf32>, %beta: tensor<?xf32>) -> tensor<32x197x?xf32> {
    ...
    %sumsq_t = tensor.empty() : tensor<32x197xf32>
    %zero_f   = arith.constant 0.0 : f32
    %sum_init = linalg.fill ins(%zero_f : f32) outs(%sum_t : tensor<32x197xf32>) -> tensor<32x197xf32>
    %sumsq_init = linalg.fill ins(%zero_f : f32) outs(%sumsq_t : tensor<32x197xf32>) -> tensor<32x197xf32>
    ...
    } ins(%input : tensor<32x197x?xf32>) outs(%sum_init, %sumsq_init : tensor<32x197xf32>, tensor<32x197xf32>) {
    ...
    } -> (tensor<32x197xf32>, tensor<32x197xf32>)
    %eps = arith.constant 1.0e-05 : f32
    %mean_t = tensor.empty() : tensor<32x197xf32>
    %istd_t = tensor.empty() : tensor<32x197xf32>
    %mean_dummy = linalg.fill ins(%zero_f : f32) outs(%mean_t : tensor<32x197xf32>) -> tensor<32x197xf32>
    %istd_dummy = linalg.fill ins(%zero_f : f32) outs(%istd_t : tensor<32x197xf32>) -> tensor<32x197xf32>
    ...
    } ins(%sum_result, %sumsq_result : tensor<32x197xf32>, tensor<32x197xf32>) outs(%mean_dummy, %istd_dummy : tensor<32x197xf32>, tensor<32x197xf32>) {
    ...
    } ins(%input, %mean_result, %istd_result, %gamma, %beta : tensor<32x197x?xf32>, tensor<32x197xf32>, tensor<32x197xf32>, tensor<?xf32>, tensor<?xf32>) outs(%out : tensor<32x197x?xf32>) {
