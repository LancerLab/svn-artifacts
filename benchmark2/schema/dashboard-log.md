...
## 2026-09-24 — §9.6.1b split: a bare hang is `never`, not `runtime`

- `mutation-specs-v2.md` §9.6.1b rule 3 says a timeout/segfault/abort with no
  emitted check is `never`, not `runtime`. The v1 classifier collapsed both into
  `runtime`, so `mlir-low`'s `M4-f` (`M4.5` zero step) hang was the lane's only
  `M4` runtime hit.
- `Classification` now carries `detected_by` (`compile-error | rtv-assert |
  gpu-assert | hang | segv | nonzero-exit | none | n/a`) alongside the unchanged
  4-value `outcome`; `outcome` keeps its vocabulary and `runtime` now means
  exactly `rt-check`.
- `schema/migrate_detected_by.py` re-expressed the committed `mlir-low` corpus
  (16 rows). `results/mlir-low/{mutants.jsonl,stats.json}` + the raw corpus were
  regenerated by `run.sh collect && run.sh stats`.
