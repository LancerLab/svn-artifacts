module {
  func.func @f_5_dynamic_Nx256x56x56_Nx256x56x56_Nx256x56x56(%lhs: tensor<?x256x56x56xf32>, %rhs: tensor<?x256x56x56xf32>) -> tensor<?x256x56x56xf32> {
    ...
    %t_res_ei0 = scf.for %ei0 = %c0 to %lhs_d0 step %c1 iter_args(%t_ea_ei0 = %out) -> (tensor<?x256x56x56xf32>) {
    %t_res_ei1 = scf.for %ei1 = %c0 to %lhs_d1 step %c1 iter_args(%t_ea_ei1 = %t_ea_ei0) -> (tensor<?x256x56x56xf32>) {
    %t_res_ei2 = scf.for %ei2 = %c0 to %lhs_d2 step %c1 iter_args(%t_ea_ei2 = %t_ea_ei1) -> (tensor<?x256x56x56xf32>) {
    %t_res_ei3 = scf.for %ei3 = %c0 to %lhs_d3 step %c1 iter_args(%t_ea_ei3 = %t_ea_ei2) -> (tensor<?x256x56x56xf32>) {
    %va = tensor.extract %lhs[%ei0, %ei1, %ei2, %ei3] : tensor<?x256x56x56xf32>
    %vb = tensor.extract %rhs[%ei0, %ei1, %ei2, %ei3] : tensor<?x256x56x56xf32>
    %rs = arith.addf %va, %vb : f32
    %t_ins = tensor.insert %rs into %t_ea_ei3[%ei0, %ei1, %ei2, %ei3] : tensor<?x256x56x56xf32>
    scf.yield %t_ins : tensor<?x256x56x56xf32>
    }
    scf.yield %t_res_ei3 : tensor<?x256x56x56xf32>
    }
    scf.yield %t_res_ei2 : tensor<?x256x56x56xf32>
    }
    scf.yield %t_res_ei1 : tensor<?x256x56x56xf32>
    }
    return %t_res_ei0 : tensor<?x256x56x56xf32>
  }
}
