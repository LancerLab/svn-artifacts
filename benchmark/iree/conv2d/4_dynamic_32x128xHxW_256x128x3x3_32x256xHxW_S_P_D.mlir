module {
  func.func @f_4_dynamic_32x128xHxW_256x128x3x3_32x256xHxW_S_P_D(%input: tensor<32x128x?x?xf32>, %filter: tensor<256x128x3x3xf32>) -> tensor<32x256x?x?xf32> {
    ...
    %filled = linalg.fill ins(%zero : f32) outs(%out : tensor<32x256x?x?xf32>) -> tensor<32x256x?x?xf32>
    %result = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>}
      ins(%input, %filter : tensor<32x128x?x?xf32>, tensor<256x128x3x3xf32>)
      outs(%filled : tensor<32x256x?x?xf32>) -> tensor<32x256x?x?xf32>
    return %result : tensor<32x256x?x?xf32>
  }
}
