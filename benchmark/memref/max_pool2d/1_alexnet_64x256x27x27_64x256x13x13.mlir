module @f_1_alexnet_64x256x27x27_64x256x13x13 {
  func.func @f_1_alexnet_64x256x27x27_64x256x13x13(%input: memref<64x256x27x27xf32>) -> memref<64x256x13x13xf32> {
    %c0 = arith.constant 0 : index
    %c1 = arith.constant 1 : index
    %c2 = arith.constant 2 : index
    %c3 = arith.constant 3 : index
    %input_d0 = memref.dim %input, %c0 : memref<64x256x27x27xf32>
    %input_d1 = memref.dim %input, %c1 : memref<64x256x27x27xf32>
    %input_d2 = memref.dim %input, %c2 : memref<64x256x27x27xf32>
    %input_d3 = memref.dim %input, %c3 : memref<64x256x27x27xf32>
    %out = memref.alloc() : memref<64x256x13x13xf32>
    %kH_sz  = arith.constant 2 : index
    %kW_sz  = arith.constant 2 : index
    %stride = arith.constant 2 : index
    %out_d2 = arith.constant 13 : index
    %out_d3 = arith.constant 13 : index
          ...
          scf.for %ow = %c0 to %out_d3 step %c1 {
            ...
            %max_v = scf.for %kh = %c0 to %kH_sz step %c1 iter_args(%mx_h = %neg_inf) -> (f32) {
              %max_v2 = scf.for %kw = %c0 to %kW_sz step %c1 iter_args(%mx_w = %mx_h) -> (f32) {
                ...
                %pv   = memref.load %input[%n, %c, %ih, %iw] : memref<64x256x27x27xf32>
            ...
            memref.store %max_v, %out[%n, %c, %oh, %ow] : memref<64x256x13x13xf32>
          }
        }
      }
    }
    return %out : memref<64x256x13x13xf32>
  }
}
