module {
  func.func @f_6_dynamic_128xCx112x112_C_C_128xCx112x112(%input: tensor<128x?x112x112xf32>, %gamma: tensor<?xf32>, %beta: tensor<?xf32>) -> tensor<128x?x112x112xf32> {
    ...
    %t_res_bn0 = scf.for %bn0 = %c0 to %input_d0 step %c1 iter_args(%t_bn_bn0 = %out) -> (tensor<128x?x112x112xf32>) {
    %t_res_bn1 = scf.for %bn1 = %c0 to %input_d1 step %c1 iter_args(%t_bn_bn1 = %t_bn_bn0) -> (tensor<128x?x112x112xf32>) {
    %t_res_bn2 = scf.for %bn2 = %c0 to %input_d2 step %c1 iter_args(%t_bn_bn2 = %t_bn_bn1) -> (tensor<128x?x112x112xf32>) {
    %t_res_bn3 = scf.for %bn3 = %c0 to %input_d3 step %c1 iter_args(%t_bn_bn3 = %t_bn_bn2) -> (tensor<128x?x112x112xf32>) {
    %xv  = tensor.extract %input[%bn0, %bn1, %bn2, %bn3] : tensor<128x?x112x112xf32>
    ...
    %t_ins = tensor.insert %res into %t_bn_bn3[%bn0, %bn1, %bn2, %bn3] : tensor<128x?x112x112xf32>
    scf.yield %t_ins : tensor<128x?x112x112xf32>
    }
    scf.yield %t_res_bn3 : tensor<128x?x112x112xf32>
    }
    scf.yield %t_res_bn2 : tensor<128x?x112x112xf32>
    }
    scf.yield %t_res_bn1 : tensor<128x?x112x112xf32>
    }
    return %t_res_bn0 : tensor<128x?x112x112xf32>
  }
}
