module {
  func.func @f_4_dynamic_Nx256x56x56_56x56_56x56(%input: tensor<?x256x56x56xf32>, %gamma: tensor<56x56xf32>, %beta: tensor<56x56xf32>) -> tensor<?x256x56x56xf32> {
    ...
    %gamma_d0 = tensor.dim %gamma, %c0 : tensor<56x56xf32>
    %gamma_d1 = tensor.dim %gamma, %c1 : tensor<56x56xf32>
    %beta_d0 = tensor.dim %beta, %c0 : tensor<56x56xf32>
    %beta_d1 = tensor.dim %beta, %c1 : tensor<56x56xf32>
    ...
    %t_res_ln_b0 = scf.for %ln_b0 = %c0 to %input_d0 step %c1 iter_args(%t_ln_ln_b0 = %out) -> (tensor<?x256x56x56xf32>) {
    %t_res_ln_b1 = scf.for %ln_b1 = %c0 to %input_d1 step %c1 iter_args(%t_ln_ln_b1 = %t_ln_ln_b0) -> (tensor<?x256x56x56xf32>) {
    ...
    %lnv1 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x256x56x56xf32>
    ...
    %lnv2 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x256x56x56xf32>
    ...
    %t_res_ln_n0 = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%t_ln_n0 = %t_ln_ln_b1) -> (tensor<?x256x56x56xf32>) {
    %t_res_ln_n1 = scf.for %ln_n1 = %c0 to %input_d3 step %c1 iter_args(%t_ln_n1 = %t_ln_n0) -> (tensor<?x256x56x56xf32>) {
    %lnv3   = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x256x56x56xf32>
    %gv     = tensor.extract %gamma[%ln_n0, %ln_n1] : tensor<56x56xf32>
    %bv     = tensor.extract %beta[%ln_n0, %ln_n1]  : tensor<56x56xf32>
    ...
    %t_ins_ln = tensor.insert %res into %t_ln_n1[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : tensor<?x256x56x56xf32>
    scf.yield %t_ins_ln : tensor<?x256x56x56xf32>
    }
    scf.yield %t_res_ln_n1 : tensor<?x256x56x56xf32>
    }
    scf.yield %t_res_ln_n0 : tensor<?x256x56x56xf32>
    }
    scf.yield %t_res_ln_b1 : tensor<?x256x56x56xf32>
    }
    return %t_res_ln_b0 : tensor<?x256x56x56xf32>
  }
}
