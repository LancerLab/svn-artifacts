module {
  func.func @f_17_mobilenet_128x96x112x112_128x96x112x112(%input: tensor<128x96x112x112xf32>) -> tensor<128x96x112x112xf32> {
    ...
    %max_t = tensor.empty() : tensor<128x96x112xf32>
    %neg_inf = arith.constant -3.4028234663852886e+38 : f32
    %max_init = linalg.fill ins(%neg_inf : f32) outs(%max_t : tensor<128x96x112xf32>) -> tensor<128x96x112xf32>
    %max_result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>],
      iterator_types = ["parallel", "parallel", "parallel", "reduction"]
    } ins(%input : tensor<128x96x112x112xf32>) outs(%max_init : tensor<128x96x112xf32>) {
    ^bb0(%in: f32, %cur: f32):
      %mx = arith.maximumf %in, %cur : f32
      linalg.yield %mx : f32
    } -> tensor<128x96x112xf32>
    %sum_t = tensor.empty() : tensor<128x96x112xf32>
    %zero_f = arith.constant 0.0 : f32
    %sum_init = linalg.fill ins(%zero_f : f32) outs(%sum_t : tensor<128x96x112xf32>) -> tensor<128x96x112xf32>
    %sum_result = linalg.generic {
      ...
      iterator_types = ["parallel", "parallel", "parallel", "reduction"]
    } ins(%input, %max_result : tensor<128x96x112x112xf32>, tensor<128x96x112xf32>) outs(%sum_init : tensor<128x96x112xf32>) {
    ...
    } ins(%input, %max_result, %sum_result : tensor<128x96x112x112xf32>, tensor<128x96x112xf32>, tensor<128x96x112xf32>) outs(%out : tensor<128x96x112x112xf32>) {
