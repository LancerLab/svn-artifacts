module @f_5_dynamic_Nx1280xHxW_NxHxWx1280 attributes {hal.device.targets = [#hal.device.target<"cuda", {executable_targets = [#hal.executable.target<"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}>], legacy_sync}>]} {
  hal.executable private @f_5_dynamic_Nx1280xHxW_NxHxWx1280_dispatch_0 {
    hal.executable.variant public @cuda_nvptx_fb, target = <"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}> {
      hal.executable.export public @f_5_dynamic_Nx1280xHxW_NxHxWx1280_dispatch_0_generic_DxDxDx1280_f32 ordinal(0) layout(#hal.pipeline.layout<push_constants = 3, sets = [<0, bindings = [<0, storage_buffer, ReadOnly>, <1, storage_buffer>]>]>) {
      ^bb0(%arg0: !hal.device, %arg1: index, %arg2: index, %arg3: index):
        %x, %y, %z = flow.dispatch.workgroup_count_from_slice %arg1, %arg2, %arg3
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @f_5_dynamic_Nx1280xHxW_NxHxWx1280_dispatch_0_generic_DxDxDx1280_f32() {
          ...
          %10 = hal.interface.binding.subspan set(0) binding(1) type(storage_buffer) alignment(64) offset(%c0) : !flow.dispatch.tensor<writeonly:tensor<?x?x?x1280xf32>>{%6, %7, %8}
          ...
          %12 = tensor.empty(%6, %7, %8) : tensor<?x?x?x1280xf32>
          %13 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d3, d1, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%11 : tensor<?x1280x?x?xf32>) outs(%12 : tensor<?x?x?x1280xf32>) {
          ^bb0(%in: f32, %out: f32):
            linalg.yield %in : f32
          } -> tensor<?x?x?x1280xf32>
          flow.dispatch.tensor.store %13, %10, offsets = [0, 0, 0, 0], sizes = [%6, %7, %8, 1280], strides = [1, 1, 1, 1] : tensor<?x?x?x1280xf32> -> !flow.dispatch.tensor<writeonly:tensor<?x?x?x1280xf32>>{%6, %7, %8}
          return
        }
      }
    }
  ...
  func.func @f_5_dynamic_Nx1280xHxW_NxHxWx1280(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    ...
    %11 = stream.cmd.execute with(%6 as %arg1: !stream.resource<external>{%5}, %7 as %arg2: !stream.resource<external>{%5}) {
      stream.cmd.dispatch @f_5_dynamic_Nx1280xHxW_NxHxWx1280_dispatch_0::@cuda_nvptx_fb::@f_5_dynamic_Nx1280xHxW_NxHxWx1280_dispatch_0_generic_DxDxDx1280_f32[%0, %1, %2](%8, %9, %10 : i32, i32, i32) {
    ...
    %13 = stream.tensor.export %12 : tensor<?x?x?x1280xf32>{%0, %1, %2} in !stream.resource<external>{%5} -> !hal.buffer_view
