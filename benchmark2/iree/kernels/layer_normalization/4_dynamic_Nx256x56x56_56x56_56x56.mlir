module {
  func.func @f_4_dynamic_Nx256x56x56_56x56_56x56(%input: tensor<?x256x56x56xf32>, %gamma: tensor<56x56xf32>, %beta: tensor<56x56xf32>) -> tensor<?x256x56x56xf32> {
    ...
    %sum_t = tensor.empty(%input_d0) : tensor<?x256xf32>
    %sumsq_t = tensor.empty(%input_d0) : tensor<?x256xf32>
    %zero_f   = arith.constant 0.0 : f32
    %sum_init = linalg.fill ins(%zero_f : f32) outs(%sum_t : tensor<?x256xf32>) -> tensor<?x256xf32>
    %sumsq_init = linalg.fill ins(%zero_f : f32) outs(%sumsq_t : tensor<?x256xf32>) -> tensor<?x256xf32>
    ...
    } ins(%input : tensor<?x256x56x56xf32>) outs(%sum_init, %sumsq_init : tensor<?x256xf32>, tensor<?x256xf32>) {
    ...
    %mean_t = tensor.empty(%input_d0) : tensor<?x256xf32>
    %istd_t = tensor.empty(%input_d0) : tensor<?x256xf32>
    %mean_dummy = linalg.fill ins(%zero_f : f32) outs(%mean_t : tensor<?x256xf32>) -> tensor<?x256xf32>
    %istd_dummy = linalg.fill ins(%zero_f : f32) outs(%istd_t : tensor<?x256xf32>) -> tensor<?x256xf32>
    ...
    } ins(%sum_result, %sumsq_result : tensor<?x256xf32>, tensor<?x256xf32>) outs(%mean_dummy, %istd_dummy : tensor<?x256xf32>, tensor<?x256xf32>) {
    ...
    } ins(%input, %mean_result, %istd_result, %gamma, %beta : tensor<?x256x56x56xf32>, tensor<?x256xf32>, tensor<?x256xf32>, tensor<56x56xf32>, tensor<56x56xf32>) outs(%out : tensor<?x256x56x56xf32>) {
