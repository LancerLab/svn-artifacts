module {
  func.func @f_8_dynamic_64x256x28x28_Cx256x3x3_64xCx28x28_S_P_D(%input: tensor<64x256x28x28xf32>, %filter: tensor<?x256x3x3xf32>) -> tensor<64x?x28x28xf32> {
    ...
    %t_res_n = scf.for %n = %c0 to %input_d0 step %c1 iter_args(%t_cv_n = %out) -> (tensor<64x?x28x28xf32>) {
    %t_res_f = scf.for %f = %c0 to %filter_d0 step %c1 iter_args(%t_cv_f = %t_cv_n) -> (tensor<64x?x28x28xf32>) {
    %t_res_oh = scf.for %oh = %c0 to %out_sz2 step %c1 iter_args(%t_cv_oh = %t_cv_f) -> (tensor<64x?x28x28xf32>) {
    %t_res_ow = scf.for %ow = %c0 to %out_sz3 step %c1 iter_args(%t_cv_ow = %t_cv_oh) -> (tensor<64x?x28x28xf32>) {
    ...
    %xv   = tensor.extract %input[%n, %c, %ih, %iw]  : tensor<64x256x28x28xf32>
    %wv   = tensor.extract %filter[%f, %c, %kh, %kw] : tensor<?x256x3x3xf32>
    ...
    %t_new = tensor.insert %acc into %t_cv_ow[%n, %f, %oh, %ow] : tensor<64x?x28x28xf32>
    scf.yield %t_new : tensor<64x?x28x28xf32>
    }
    scf.yield %t_res_ow : tensor<64x?x28x28xf32>
    }
    scf.yield %t_res_oh : tensor<64x?x28x28xf32>
    }
    scf.yield %t_res_f : tensor<64x?x28x28xf32>
    }
    return %t_res_n : tensor<64x?x28x28xf32>
  }
}
