module {
  func.func @f_9_dynamic_64x128xHxW_HxW_HxW(%input: tensor<64x128x?x?xf32>, %gamma: tensor<?x?xf32>, %beta: tensor<?x?xf32>) -> tensor<64x128x?x?xf32> {
    ...
    %sumsq_t = tensor.empty() : tensor<64x128xf32>
    %zero_f   = arith.constant 0.0 : f32
    %sum_init = linalg.fill ins(%zero_f : f32) outs(%sum_t : tensor<64x128xf32>) -> tensor<64x128xf32>
    %sumsq_init = linalg.fill ins(%zero_f : f32) outs(%sumsq_t : tensor<64x128xf32>) -> tensor<64x128xf32>
    ...
    } ins(%input : tensor<64x128x?x?xf32>) outs(%sum_init, %sumsq_init : tensor<64x128xf32>, tensor<64x128xf32>) {
    ...
    %mean_t = tensor.empty() : tensor<64x128xf32>
    %istd_t = tensor.empty() : tensor<64x128xf32>
    %mean_dummy = linalg.fill ins(%zero_f : f32) outs(%mean_t : tensor<64x128xf32>) -> tensor<64x128xf32>
    %istd_dummy = linalg.fill ins(%zero_f : f32) outs(%istd_t : tensor<64x128xf32>) -> tensor<64x128xf32>
    ...
    } ins(%sum_result, %sumsq_result : tensor<64x128xf32>, tensor<64x128xf32>) outs(%mean_dummy, %istd_dummy : tensor<64x128xf32>, tensor<64x128xf32>) {
    ...
    } ins(%input, %mean_result, %istd_result, %gamma, %beta : tensor<64x128x?x?xf32>, tensor<64x128xf32>, tensor<64x128xf32>, tensor<?x?xf32>, tensor<?x?xf32>) outs(%out : tensor<64x128x?x?xf32>) {
