module {
  func.func @f_16_lstm_64x100x256_256_256(%input: tensor<64x100x256xf32>, %gamma: tensor<256xf32>, %beta: tensor<256xf32>) -> tensor<64x100x256xf32> {
    ...
    %t_res_ln_b0 = scf.for %ln_b0 = %c0 to %input_d0 step %c1 iter_args(%t_ln_ln_b0 = %out) -> (tensor<64x100x256xf32>) {
    %t_res_ln_b1 = scf.for %ln_b1 = %c0 to %input_d1 step %c1 iter_args(%t_ln_ln_b1 = %t_ln_ln_b0) -> (tensor<64x100x256xf32>) {
    ...
    %lnv1 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0] : tensor<64x100x256xf32>
    %ns1  = arith.addf %ln_sa0, %lnv1 : f32
    scf.yield %ns1 : f32
    }
    %ln_sumsq = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%ln_ssa0 = %zero_sf) -> (f32) {
    %lnv2 = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0] : tensor<64x100x256xf32>
    ...
    %t_res_ln_n0 = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%t_ln_n0 = %t_ln_ln_b1) -> (tensor<64x100x256xf32>) {
    %lnv3   = tensor.extract %input[%ln_b0, %ln_b1, %ln_n0] : tensor<64x100x256xf32>
    %gv     = tensor.extract %gamma[%ln_n0] : tensor<256xf32>
    %bv     = tensor.extract %beta[%ln_n0]  : tensor<256xf32>
    ...
    %t_ins_ln = tensor.insert %res into %t_ln_n0[%ln_b0, %ln_b1, %ln_n0] : tensor<64x100x256xf32>
    scf.yield %t_ins_ln : tensor<64x100x256xf32>
    }
    scf.yield %t_res_ln_n0 : tensor<64x100x256xf32>
    }
    scf.yield %t_res_ln_b1 : tensor<64x100x256xf32>
    }
    return %t_res_ln_b0 : tensor<64x100x256xf32>
  }
}
