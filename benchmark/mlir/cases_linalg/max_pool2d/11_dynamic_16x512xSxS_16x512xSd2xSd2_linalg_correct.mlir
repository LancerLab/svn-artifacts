module {
  func.func @f_11_dynamic_16x512xSxS_16x512xSd2xSd2_correct(%input: tensor<16x512x?x?xf32>) -> tensor<16x512x?x?xf32> {
    ...
    %init = tensor.empty(%od2, %od3) : tensor<16x512x?x?xf32>
    %fill = linalg.fill ins(%neg_inf : f32) outs(%init : tensor<16x512x?x?xf32>) -> tensor<16x512x?x?xf32>
    ...
    %r = linalg.pooling_nchw_max {dilations = dense<1> : tensor<2xi64>, strides = dense<1> : tensor<2xi64>}
         ...
         outs(%fill : tensor<16x512x?x?xf32>) -> tensor<16x512x?x?xf32>
