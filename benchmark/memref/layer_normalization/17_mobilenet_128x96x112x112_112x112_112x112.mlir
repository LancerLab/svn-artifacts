module @f_17_mobilenet_128x96x112x112_112x112_112x112 {
  func.func @f_17_mobilenet_128x96x112x112_112x112_112x112(%input: memref<128x96x112x112xf32>, %gamma: memref<112x112xf32>, %beta: memref<112x112xf32>) -> memref<128x96x112x112xf32> {
    ...
    %gamma_d0 = memref.dim %gamma, %c0 : memref<112x112xf32>
    %gamma_d1 = memref.dim %gamma, %c1 : memref<112x112xf32>
    %beta_d0 = memref.dim %beta, %c0 : memref<112x112xf32>
    %beta_d1 = memref.dim %beta, %c1 : memref<112x112xf32>
    ...
    scf.for %ln_b0 = %c0 to %input_d0 step %c1 {
      scf.for %ln_b1 = %c0 to %input_d1 step %c1 {
        ...
        %ln_sum = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%ln_sa0 = %zero_f) -> (f32) {
            ...
            %lnv1 = memref.load %input[%ln_b0, %ln_b1, %ln_n0, %ln_n1] : memref<128x96x112x112xf32>
        ...
        %ln_sumsq = scf.for %ln_n0v = %c0 to %input_d2 step %c1 iter_args(%ln_sq0 = %zero_f) -> (f32) {
            ...
            %lnv2 = memref.load %input[%ln_b0, %ln_b1, %ln_n0v, %ln_n1v] : memref<128x96x112x112xf32>
        ...
        scf.for %ln_w0 = %c0 to %input_d2 step %c1 {
          scf.for %ln_w1 = %c0 to %input_d3 step %c1 {
            %lnv3    = memref.load %input[%ln_b0, %ln_b1, %ln_w0, %ln_w1] : memref<128x96x112x112xf32>
            %lnd3    = arith.subf %lnv3, %ln_mean : f32
            %ln_norm = arith.mulf %lnd3, %ln_invstd : f32
            %gv      = memref.load %gamma[%ln_w0, %ln_w1] : memref<112x112xf32>
            %bv      = memref.load %beta[%ln_w0, %ln_w1]  : memref<112x112xf32>
            %scaled  = arith.mulf %ln_norm, %gv : f32
            %ln_res  = arith.addf %scaled, %bv : f32
            memref.store %ln_res, %out[%ln_b0, %ln_b1, %ln_w0, %ln_w1] : memref<128x96x112x112xf32>
          }
        }
      }
    }
