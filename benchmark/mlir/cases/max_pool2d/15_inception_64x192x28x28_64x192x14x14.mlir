module {
  func.func @f_15_inception_64x192x28x28_64x192x14x14(%input: tensor<64x192x28x28xf32>) -> tensor<64x192x14x14xf32> {
    ...
    %t_res_pn = scf.for %pn = %c0 to %input_d0 step %c1 iter_args(%t_mp_pn = %out) -> (tensor<64x192x14x14xf32>) {
    %t_res_pc = scf.for %pc = %c0 to %input_d1 step %c1 iter_args(%t_mp_pc = %t_mp_pn) -> (tensor<64x192x14x14xf32>) {
    %t_res_poh = scf.for %poh = %c0 to %po_sz2 step %c1 iter_args(%t_mp_poh = %t_mp_pc) -> (tensor<64x192x14x14xf32>) {
    %t_res_pow = scf.for %pow = %c0 to %po_sz3 step %c1 iter_args(%t_mp_pow = %t_mp_poh) -> (tensor<64x192x14x14xf32>) {
    ...
    %pv    = tensor.extract %input[%pn, %pc, %ih, %iw] : tensor<64x192x28x28xf32>
    ...
    %t_new = tensor.insert %pool into %t_mp_pow[%pn, %pc, %poh, %pow] : tensor<64x192x14x14xf32>
    scf.yield %t_new : tensor<64x192x14x14xf32>
    }
    scf.yield %t_res_pow : tensor<64x192x14x14xf32>
    }
    scf.yield %t_res_poh : tensor<64x192x14x14xf32>
    }
    scf.yield %t_res_pc : tensor<64x192x14x14xf32>
    }
    return %t_res_pn : tensor<64x192x14x14xf32>
  }
}
