module @f_1_bert_32x512x768 attributes {hal.device.targets = [#hal.device.target<"cuda", {executable_targets = [#hal.executable.target<"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}>], legacy_sync}>]} {
  hal.executable private @f_1_bert_32x512x768_dispatch_0 {
    hal.executable.variant public @cuda_nvptx_fb, target = <"cuda", "cuda-nvptx-fb", {target_arch = "sm_60"}> {
      hal.executable.export public @f_1_bert_32x512x768_dispatch_0_generic_12582912_f32 ordinal(0) layout(#hal.pipeline.layout<push_constants = 0, sets = [<0, bindings = [<0, storage_buffer, ReadOnly>, <1, storage_buffer>]>]>) {
      ^bb0(%arg0: !hal.device):
        %x, %y, %z = flow.dispatch.workgroup_count_from_slice 
        hal.return %x, %y, %z : index, index, index
      }
      builtin.module {
        func.func @f_1_bert_32x512x768_dispatch_0_generic_12582912_f32() {
  ...
  func.func @f_1_bert_32x512x768(%arg0: !hal.buffer_view) -> !hal.buffer_view attributes {iree.abi.stub} {
    ...
    %2 = stream.cmd.execute with(%0 as %arg1: !stream.resource<external>{%c50331648}, %1 as %arg2: !stream.resource<external>{%c50331648}) {
      stream.cmd.dispatch @f_1_bert_32x512x768_dispatch_0::@cuda_nvptx_fb::@f_1_bert_32x512x768_dispatch_0_generic_12582912_f32 {
