      ...
      scf.for %so1 = %c0 to %input_d1 step %c1 {
        scf.for %so2 = %c0 to %input_d2 step %c1 {
          ...
          %max_val = scf.for %sk = %c0 to %input_d3 step %c1 iter_args(%mx = %neg_inf) -> (f32) {
            %pv1 = memref.load %input[%so0, %so1, %so2, %sk] : memref<128x96x112x112xf32>
          ...
          %sum_val = scf.for %sk2 = %c0 to %input_d3 step %c1 iter_args(%sm = %zero_f) -> (f32) {
            %pv2     = memref.load %input[%so0, %so1, %so2, %sk2] : memref<128x96x112x112xf32>
          ...
          scf.for %sk3 = %c0 to %input_d3 step %c1 {
            %pv3  = memref.load %input[%so0, %so1, %so2, %sk3] : memref<128x96x112x112xf32>
            %sh3  = arith.subf %pv3, %max_val : f32
            %ex3  = math.exp %sh3 : f32
            %ov   = arith.divf %ex3, %sum_val : f32
            memref.store %ov, %out[%so0, %so1, %so2, %sk3] : memref<128x96x112x112xf32>
          }
        }
      }
    }
