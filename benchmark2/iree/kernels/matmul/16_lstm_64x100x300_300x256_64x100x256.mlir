module {
  func.func @f_16_lstm_64x100x300_300x256_64x100x256(%lhs: tensor<64x100x300xf32>, %rhs: tensor<300x256xf32>) -> tensor<64x100x256xf32> {
    ...
    %filled = linalg.fill ins(%zero : f32) outs(%out : tensor<64x100x256xf32>) -> tensor<64x100x256xf32>
    %result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d3)>, affine_map<(d0, d1, d2, d3) -> (d3, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>],
      iterator_types = ["parallel", "parallel", "parallel", "reduction"]
    } ins(%lhs, %rhs : tensor<64x100x300xf32>, tensor<300x256xf32>) outs(%filled : tensor<64x100x256xf32>) {
