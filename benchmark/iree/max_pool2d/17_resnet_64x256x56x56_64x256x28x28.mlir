module {
  func.func @f_17_resnet_64x256x56x56_64x256x28x28(%input: tensor<64x256x56x56xf32>) -> tensor<64x256x28x28xf32> {
    ...
    %filled = linalg.fill ins(%neg_inf : f32) outs(%out : tensor<64x256x28x28xf32>) -> tensor<64x256x28x28xf32>
    ...
    %result = linalg.pooling_nchw_max {dilations = dense<1> : vector<2xi64>, strides = dense<2> : vector<2xi64>}
      ins(%input, %kernel : tensor<64x256x56x56xf32>, tensor<2x2xf32>)
      outs(%filled : tensor<64x256x28x28xf32>) -> tensor<64x256x28x28xf32>
