...
M2 = [
    ...
    _m("M2.5.rl1.dup", "M2", 5, "wrong-shape", "relu",
       "1_bert_32x512x768_32x512x768",
       "duplicate write: every lane of the parallel fan-in overwrites the first shared staging slot on relu (second cell: the same defect on another kernel of the family)",
       ("out_s.at(0, 0, q, 0) = (inp_s.at(0, 0, q, 0) > 0.0f) ? inp_s.at(0, 0, q, 0) : 0.0f;",
        "out_s.at(0, 0, 0, 0) = (inp_s.at(0, 0, q, 0) > 0.0f) ? inp_s.at(0, 0, q, 0) : 0.0f;")),
    _m("M2.5.rl1.partial", "M2", 5, "wrong-shape", "relu",
       "1_bert_32x512x768_32x512x768",
       "partial write: the tail tile of the block loop is never staged on relu (second cell: the same defect on another kernel of the family)",
       ("foreach {i, j, k} in [I / #p, J, 12] {",
        "foreach {i, j, k} in [I / #p, J, 11] {")),
    _m("M2.5.tp1.dup", "M2", 5, "wrong-shape", "transpose",
       "1_bert_32x512x768_32x768x512",
       "duplicate write: every lane of the parallel fan-in overwrites the first shared staging slot on transpose (second cell: the same defect on another kernel of the family)",
       ("os.at(0, q, 0) = is.at(0, 0, q);",
        "os.at(0, 0, 0) = is.at(0, 0, q);")),
    _m("M2.5.tp1.partial", "M2", 5, "wrong-shape", "transpose",
       "1_bert_32x512x768_32x768x512",
       "partial write: the tail tile of the block loop is never staged on transpose (second cell: the same defect on another kernel of the family)",
       ("foreach {y, z} in [512, 12] {",
        "foreach {y, z} in [512, 11] {")),
    _m("M2.5.sm1.dup", "M2", 5, "wrong-shape", "softmax",
       "1_bert_32x512x768_32x512x768",
       "duplicate write: every batch row is staged into the first output block on softmax (second cell: the same defect on another kernel of the family)",
       ("dma.copy l1_out => output.chunkat(i, p#q, _);",
        "dma.copy l1_out => output.chunkat(0, p#q, _);")),
    _m("M2.5.sm1.partial", "M2", 5, "wrong-shape", "softmax",
       "1_bert_32x512x768_32x512x768",
       "partial write: the last row of the batch is never staged on softmax (second cell: the same defect on another kernel of the family)",
       ("foreach {i} in [input.span(0)] {",
        "foreach {i} in [input.span(0) - 1] {")),
]
...
M2 += [
    ...
    _m("M2.17.ln6.rtshaped", "M2", 17, "dim-mismatch", "layer_normalization",
       "6_dynamic_128xCx112x112_112x112_112x112",
       "runtime-shaped operand (N0 symbolic): the declared extent error escapes the size check entirely (second cell: the same defect on another kernel of the family)",
       ("f32 [I, N0, K, L] lhs", "f32 [I, N0, K, L + 1] lhs")),
    _m("M2.17.rp5.rtshaped", "M2", 17, "dim-mismatch", "reshape",
       "5_dynamic_Bx1280xHxW_Bx1280HW",
       "runtime-shaped operand (height/width symbolic): a wrong declared extent escapes the size check on reshape",
       ("f32 [batch_size, 1280, height, width] input",
        "f32 [batch_size, 1280, height, width + 1] input")),
    _m("M2.17.gl11.rtshaped", "M2", 17, "dim-mismatch", "gelu",
       "11_dynamic_32xSx768",
       "runtime-shaped operand (seq_len symbolic): a wrong declared extent escapes the size check on gelu",
       ("f32 [32, seq_len, 768] i", "f32 [32, seq_len + 1, 768] i")),
    _m("M2.17.sm11.rtshaped", "M2", 17, "dim-mismatch", "softmax",
       "11_dynamic_32xSx768_32xSx768",
       "runtime-shaped operand (seq_len symbolic): a wrong declared extent escapes the size check on softmax",
       ("f32 [32, seq_len, 768] input", "f32 [32, seq_len + 1, 768] input")),
    _m("M2.17.mm11.rtshaped", "M2", 17, "dim-mismatch", "matmul",
       "11_dynamic_32xSx768_768x768_32xSx768",
       "runtime-shaped operand (S symbolic): a wrong declared inner extent escapes the size check on matmul",
       ("f32 [32, S, 768] lhs", "f32 [32, S, 769] lhs")),
    _m("M2.17.tp11.rtshaped", "M2", 17, "dim-mismatch", "transpose",
       "11_dynamic_32xSx768_32x768xS",
       "runtime-shaped inner dimension (seq_len symbolic): a wrong declared extent escapes the size check on transpose",
       ("f32 [32, seq_len, 768] i", "f32 [32, seq_len + 1, 768] i")),
    _m("M2.17.cv2.rtshaped", "M2", 17, "dim-mismatch", "conv2d",
       "2_dynamic_Nx64x56x56_128x64x3x3_Nx128x56x56_S_P_D",
       "runtime-shaped batch dimension (batch_size symbolic): a wrong declared extent escapes the size check on conv2d",
       ("f32 [batch_size, 64, 56, 56] i", "f32 [batch_size, 64, 56, 57] i")),
]
