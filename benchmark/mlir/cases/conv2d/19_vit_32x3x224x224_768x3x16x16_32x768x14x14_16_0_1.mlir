module {
  func.func @f_19_vit_32x3x224x224_768x3x16x16_32x768x14x14_16_0_1(%input: tensor<32x3x224x224xf32>, %filter: tensor<768x3x16x16xf32>) -> tensor<32x768x14x14xf32> {
    ...
    %out_sz2 = arith.constant 14 : index
    %out_sz3 = arith.constant 14 : index
    %t_res_n = scf.for %n = %c0 to %input_d0 step %c1 iter_args(%t_cv_n = %out) -> (tensor<32x768x14x14xf32>) {
    %t_res_f = scf.for %f = %c0 to %filter_d0 step %c1 iter_args(%t_cv_f = %t_cv_n) -> (tensor<32x768x14x14xf32>) {
    %t_res_oh = scf.for %oh = %c0 to %out_sz2 step %c1 iter_args(%t_cv_oh = %t_cv_f) -> (tensor<32x768x14x14xf32>) {
    %t_res_ow = scf.for %ow = %c0 to %out_sz3 step %c1 iter_args(%t_cv_ow = %t_cv_oh) -> (tensor<32x768x14x14xf32>) {
    ...
    %xv   = tensor.extract %input[%n, %c, %ih, %iw]  : tensor<32x3x224x224xf32>
    %wv   = tensor.extract %filter[%f, %c, %kh, %kw] : tensor<768x3x16x16xf32>
    ...
    %t_new = tensor.insert %acc into %t_cv_ow[%n, %f, %oh, %ow] : tensor<32x768x14x14xf32>
    scf.yield %t_new : tensor<32x768x14x14xf32>
    }
    scf.yield %t_res_ow : tensor<32x768x14x14xf32>
    }
    scf.yield %t_res_oh : tensor<32x768x14x14xf32>
    }
    scf.yield %t_res_f : tensor<32x768x14x14xf32>
    }
    return %t_res_n : tensor<32x768x14x14xf32>
  }
}
