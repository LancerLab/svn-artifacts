module @f_16_lstm_64x100x256_256_256 {
  func.func @f_16_lstm_64x100x256_256_256(%input: memref<64x100x256xf32>, %gamma: memref<256xf32>, %beta: memref<256xf32>) -> memref<64x100x256xf32> {
    ...
    scf.for %ln_b0 = %c0 to %input_d0 step %c1 {
      scf.for %ln_b1 = %c0 to %input_d1 step %c1 {
        ...
        %ln_sum = scf.for %ln_n0 = %c0 to %input_d2 step %c1 iter_args(%ln_sa0 = %zero_f) -> (f32) {
          %lnv1 = memref.load %input[%ln_b0, %ln_b1, %ln_n0] : memref<64x100x256xf32>
        ...
        %ln_sumsq = scf.for %ln_n0v = %c0 to %input_d2 step %c1 iter_args(%ln_sq0 = %zero_f) -> (f32) {
          %lnv2 = memref.load %input[%ln_b0, %ln_b1, %ln_n0v] : memref<64x100x256xf32>
        ...
        scf.for %ln_w0 = %c0 to %input_d2 step %c1 {
          %lnv3    = memref.load %input[%ln_b0, %ln_b1, %ln_w0] : memref<64x100x256xf32>
          %lnd3    = arith.subf %lnv3, %ln_mean : f32
          %ln_norm = arith.mulf %lnd3, %ln_invstd : f32
          %gv      = memref.load %gamma[%ln_w0] : memref<256xf32>
          %bv      = memref.load %beta[%ln_w0]  : memref<256xf32>
          %scaled  = arith.mulf %ln_norm, %gv : f32
          %ln_res  = arith.addf %scaled, %bv : f32
          memref.store %ln_res, %out[%ln_b0, %ln_b1, %ln_w0] : memref<64x100x256xf32>
        }
      }
    }
