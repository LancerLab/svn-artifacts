module {
  func.func @f_3_dynamic_16x256xHxW_256x256x3x3_16x256xHxW_S_P_D(%input: tensor<16x256x?x?xf32>, %filter: tensor<256x256x3x3xf32>) -> tensor<16x256x?x?xf32> {
    ...
    %t_res_n = scf.for %n = %c0 to %input_d0 step %c1 iter_args(%t_cv_n = %out) -> (tensor<16x256x?x?xf32>) {
    %t_res_f = scf.for %f = %c0 to %filter_d0 step %c1 iter_args(%t_cv_f = %t_cv_n) -> (tensor<16x256x?x?xf32>) {
    %t_res_oh = scf.for %oh = %c0 to %input_d2 step %c1 iter_args(%t_cv_oh = %t_cv_f) -> (tensor<16x256x?x?xf32>) {
    %t_res_ow = scf.for %ow = %c0 to %input_d3 step %c1 iter_args(%t_cv_ow = %t_cv_oh) -> (tensor<16x256x?x?xf32>) {
    ...
    %xv   = tensor.extract %input[%n, %c, %ih, %iw]  : tensor<16x256x?x?xf32>
    %wv   = tensor.extract %filter[%f, %c, %kh, %kw] : tensor<256x256x3x3xf32>
    ...
    %t_new = tensor.insert %acc into %t_cv_ow[%n, %f, %oh, %ow] : tensor<16x256x?x?xf32>
    scf.yield %t_new : tensor<16x256x?x?xf32>
    }
    scf.yield %t_res_ow : tensor<16x256x?x?xf32>
    }
    scf.yield %t_res_oh : tensor<16x256x?x?xf32>
    }
    scf.yield %t_res_f : tensor<16x256x?x?xf32>
    }
    return %t_res_n : tensor<16x256x?x?xf32>
  }
}
