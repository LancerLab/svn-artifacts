module {
  func.func @f_18_resnet_128x2048_2048x1000_128x1000(%lhs: tensor<128x2048xf32>, %rhs: tensor<2048x1000xf32>) -> tensor<128x1000xf32> {
    ...
    %t_res_i = scf.for %i = %c0 to %lhs_d0 step %c1 iter_args(%t_arg_i = %out) -> (tensor<128x1000xf32>) {
    %t_res_j = scf.for %j = %c0 to %rhs_d1 step %c1 iter_args(%t_arg_j = %t_arg_i) -> (tensor<128x1000xf32>) {
    %acc = scf.for %k = %c0 to %lhs_d1 step %c1 iter_args(%s = %zero_f) -> (f32) {
    %a   = tensor.extract %lhs[%i, %k] : tensor<128x2048xf32>
    %b   = tensor.extract %rhs[%k, %j] : tensor<2048x1000xf32>
    %p   = arith.mulf %a, %b : f32
    %ns  = arith.addf %s, %p : f32
    scf.yield %ns : f32
    }
    %t_new = tensor.insert %acc into %t_arg_j[%i, %j] : tensor<128x1000xf32>
    scf.yield %t_new : tensor<128x1000xf32>
    }
    scf.yield %t_res_j : tensor<128x1000xf32>
    }
    return %t_res_i : tensor<128x1000xf32>
  }
}
