module {
  func.func @f_6_dynamic_64x64x56x56_128x64x3x3_64x128x56x56_S_P_D(%input: tensor<64x64x56x56xf32>, %filter: tensor<128x64x3x3xf32>) -> tensor<64x128x56x56xf32> {
    ...
    } : tensor<64x64x56x56xf32> to tensor<64x64x58x58xf32>
    %zero = arith.constant 0.0 : f32
    %filled = linalg.fill ins(%zero : f32) outs(%out : tensor<64x128x56x56xf32>) -> tensor<64x128x56x56xf32>
    %result = linalg.conv_2d_nchw_fchw {dilations = dense<1> : vector<2xi64>, strides = dense<1> : vector<2xi64>}
      ins(%padded, %filter : tensor<64x64x58x58xf32>, tensor<128x64x3x3xf32>)
      outs(%filled : tensor<64x128x56x56xf32>) -> tensor<64x128x56x56xf32>
