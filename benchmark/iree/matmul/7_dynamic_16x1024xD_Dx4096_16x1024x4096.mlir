module {
  func.func @f_7_dynamic_16x1024xD_Dx4096_16x1024x4096(%lhs: tensor<16x1024x?xf32>, %rhs: tensor<?x4096xf32>) -> tensor<16x1024x4096xf32> {
    ...
    %filled = linalg.fill ins(%zero : f32) outs(%out : tensor<16x1024x4096xf32>) -> tensor<16x1024x4096xf32>
    %result = linalg.generic {
      indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d3)>, affine_map<(d0, d1, d2, d3) -> (d3, d2)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2)>],
      iterator_types = ["parallel", "parallel", "parallel", "reduction"]
    } ins(%lhs, %rhs : tensor<16x1024x?xf32>, tensor<?x4096xf32>) outs(%filled : tensor<16x1024x4096xf32>) {
