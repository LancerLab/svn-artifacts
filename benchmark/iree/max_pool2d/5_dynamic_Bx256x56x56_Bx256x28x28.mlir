module {
  func.func @f_5_dynamic_Bx256x56x56_Bx256x28x28(%input: tensor<?x256x56x56xf32>) -> tensor<?x256x28x28xf32> {
    ...
    %filled = linalg.fill ins(%neg_inf : f32) outs(%out : tensor<?x256x28x28xf32>) -> tensor<?x256x28x28xf32>
    ...
    %result = linalg.pooling_nchw_max {dilations = dense<1> : vector<2xi64>, strides = dense<2> : vector<2xi64>}
      ins(%input, %kernel : tensor<?x256x56x56xf32>, tensor<2x2xf32>)
      outs(%filled : tensor<?x256x28x28xf32>) -> tensor<?x256x28x28xf32>
    return %result : tensor<?x256x28x28xf32>
  }
}
